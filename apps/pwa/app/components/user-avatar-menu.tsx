...
const UserAvatarMenu = (props: UserAvatarMenuProps): React.JSX.Element => {
    ...
    return (<div ref={rootRef} className="relative">
      <button type="button" onClick={(): void => {
        ...
        }} className={cn("inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/80 px-2 py-1.5 text-sm text-zinc-700 hover:bg-zinc-50 dark:border-white/10 dark:bg-black/40 dark:text-zinc-200 dark:hover:bg-zinc-900/40", props.compact ? "px-1.5" : "")} aria-label="Open profile menu" aria-expanded={open}>
      ...
      {open ? (<div className={cn("absolute z-50 w-56 overflow-hidden rounded-xl border border-black/10 bg-white shadow-lg dark:border-white/10 dark:bg-zinc-950", props.alignStart ? "left-0" : "right-0", openUp ? "bottom-full mb-2" : "top-full mt-2")}>
          ...
          <Link href="/settings#profile" className="block px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-50 dark:text-zinc-200 dark:hover:bg-white/5" onClick={(): void => setOpen(false)}>
            {t("settings.tabs.profile")}
          </Link>
          <Link href="/settings" className="block px-3 py-2 text-sm text-zinc-700 hover:bg-zinc-50 dark:text-zinc-200 dark:hover:bg-white/5" onClick={(): void => setOpen(false)}>
            {t("settings.title")}
          </Link>
          {canLock ? (
            <button
              type="button"
              className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-zinc-700 hover:bg-zinc-50 dark:text-zinc-200 dark:hover:bg-white/5"
              onClick={(): void => {
                setOpen(false);
                openLockConfirm();
              }}
          ...
          <button type="button" className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/30" onClick={async (): Promise<void> => {
                setOpen(false);
                await authKernel.signOutBoundProfileWindow(desktopSnapshot.currentWindow.profileId);
                router.replace("/");
            }}>
