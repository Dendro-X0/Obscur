...
type InboxState =
  | { status: "loading" }
  | { status: "loaded"; requests: ConnectionRequest[] }
...
export const ConnectionRequestInbox = () => {
  ...
  const loadRequests = async () => {
    try {
      const requests = await inviteManager.getIncomingConnectionRequests();
      setState({ status: "loaded", requests });
    } catch (error) {
      setState({
        status: "error",
        error: error instanceof Error ? error.message : "Failed to load connection requests"
      });
    }
  };
  ...
  const handleAccept = async (requestId: string) => {
    ...
    try {
      await inviteManager.acceptConnectionRequest(requestId);
  ...
  const handleDecline = async (requestId: string, block: boolean = false) => {
    ...
    try {
      await inviteManager.declineConnectionRequest(requestId, block);
      await loadRequests(); // Reload the list
      // TODO: Show success toast
    } catch (error) {
  ...
  if (state.status === "error") {
    return (
      <Card title="Connection Requests" description="Manage incoming connection requests" tone="danger">
