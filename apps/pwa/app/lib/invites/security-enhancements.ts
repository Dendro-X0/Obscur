...
export class InputValidator {
   ...
   */
  static validatePublicKey(publicKey: string): { isValid: boolean; normalized?: PublicKeyHex; error?: string } {
    if (typeof publicKey !== 'string') {
      return { isValid: false, error: 'Public key must be a string' };
    }
    
    try {
      const normalized = cryptoService.normalizeKey(publicKey);
      
      if (!cryptoService.isValidPubkey(normalized)) {
