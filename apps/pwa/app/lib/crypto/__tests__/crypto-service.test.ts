...
import type { InviteData } from '../crypto-service';
...
describe('CryptoService Property Tests', () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(nip04Encrypt).mockImplementation(async ({ plaintext, senderPrivateKeyHex, recipientPublicKeyHex }: any) => {
      // Create a deterministic but unique encryption based on inputs
      const keyCombo = senderPrivateKeyHex + recipientPublicKeyHex;
      const hash = keyCombo.slice(0, 8); // Use first 8 chars as identifier
      return `encrypted_${hash}_${plaintext.slice(0, 10)}_${mockRandomCounter++}`;
    });
    ...
    vi.mocked(nip04Decrypt).mockImplementation(async ({ payload, senderPublicKeyHex, recipientPrivateKeyHex }: any) => {
      if (payload.startsWith('encrypted_')) {
        ...
        if (parts.length >= 4) {
          // Verify the key combination matches
          const expectedKeyCombo = recipientPrivateKeyHex + senderPublicKeyHex;
          const expectedHash = expectedKeyCombo.slice(0, 8);
          
          if (parts[1] === expectedHash) {
            // Extract the original message from the encrypted payload
            // Find the original message by looking for it in the test messages
            const testMessages = [
              'Hello, world!',
              ...
              'Test message for non-deterministic encryption',
              'Secret message',
              'Message 1',
              'Message 2', 
              'Message 3'
            ...
            // Find the message that matches the encrypted prefix
            const messagePrefix = parts[2];
            const matchingMessage = testMessages.find(msg => msg.slice(0, 10) === messagePrefix);
            
            if (matchingMessage) {
              return matchingMessage;
            }
            
            // Fallback: return a reconstructed message
            return `${messagePrefix}...`; // Truncated version
          }
        }
      }
      throw new Error('Invalid ciphertext or wrong keys');
    });
    ...
    vi.mocked(schnorr.sign).mockImplementation(async (hash: string, privateKey: string) => {
    ...
    vi.mocked(schnorr.verify).mockImplementation((signature: string, hash: string, publicKey: string) => {
...
describe('Invite Crypto Operations Property Tests', () => {
  // Test data - valid 64-character hex strings (32 bytes) - FIXED: removed extra characters
  const validPublicKey: PublicKeyHex = 'a1633cafcc01ebfb6d78e39f687a1f0995c62fc95f51ead10a02ee0be551b5dc' as PublicKeyHex;
  const validPrivateKey: PrivateKeyHex = '5dab087e624a8a4b79e17f8b83800ee66f3bb1292618b6fd1c2f8b27ff88e0eb' as PrivateKeyHex;
  const anotherPublicKey: PublicKeyHex = 'c2047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee5' as PublicKeyHex;
  const anotherPrivateKey: PrivateKeyHex = '6dab087e624a8a4b79e17f8b83800ee66f3bb1292618b6fd1c2f8b27ff88e0ec' as PrivateKeyHex;
  ...
  const validInviteData: InviteData = {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.record({
            ...
            message: fc.option(fc.string({ minLength: 0, maxLength: 500 })),
  ...
  describe('Property 19: Cryptographic Security', () => {
    ...
    it('should generate cryptographically secure random bytes', async () => {
      await fc.assert(
        fc.property(
          ...
          (length: number) => {
            const randomBytes = cryptoService.generateSecureRandom(length);
    ...
    it('should produce different random bytes on each call', () => {
      const length = 32;
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (data: string) => {
            ...
            const key = cryptoService.generateSecureRandom(32);
    ...
    it('should fail decryption with wrong key', async () => {
      ...
      const correctKey = cryptoService.generateSecureRandom(32);
      const wrongKey = cryptoService.generateSecureRandom(32);
    ...
    it('should validate encryption key requirements', async () => {
      ...
      const validKey = cryptoService.generateSecureRandom(32);
...
describe('Security Hardening Tests', () => {
  describe('Memory Cleanup (Requirement 9.6)', () => {
    ...
    it('should handle null and invalid buffer inputs gracefully', () => {
      // Should not throw for invalid inputs
      expect(() => cryptoService.security.clearSensitiveBuffer(null as any)).not.toThrow();
      expect(() => cryptoService.security.clearSensitiveBuffer(undefined as any)).not.toThrow();
      expect(() => cryptoService.security.clearSensitiveBuffer('not a buffer' as any)).not.toThrow();
    });

    it('should clear buffers of various sizes', () => {
      ...
      for (const size of sizes) {
        ...
        cryptoService.security.clearSensitiveBuffer(buffer);
  ...
  describe('Timing Attack Prevention (Requirement 9.7)', () => {
    it('should perform constant-time comparison for equal byte arrays', () => {
    ...
    it('should perform constant-time comparison for different byte arrays', () => {
      const data1 = new Uint8Array([1, 2, 3, 4, 5]);
    ...
    it('should detect string differences in constant time', () => {
      const str1 = 'secret_password_123';
    ...
    it('should handle invalid inputs for constant-time comparison', () => {
    ...
    it('should take similar time for equal and unequal comparisons', () => {
      ...
      for (let i = 0; i < 100; i++) {
        cryptoService.security.constantTimeCompare(data1, data2Equal);
      }
      ...
      for (let i = 0; i < 100; i++) {
        cryptoService.security.constantTimeCompare(data1, data2Different);
      }
  ...
  describe('Secure Logging (Requirement 9.8)', () => {
    it('should redact sensitive fields from objects', () => {
      ...
      const sanitized = cryptoService.security.sanitizeForLogging(sensitiveData);
    ...
    it('should handle nested objects', () => {
      ...
      const sanitized = cryptoService.security.sanitizeForLogging(nestedData);
    ...
    it('should handle arrays', () => {
      ...
      const sanitized = cryptoService.security.sanitizeForLogging(arrayData);
    ...
    it('should handle Error objects', () => {
      ...
      const sanitized = cryptoService.security.sanitizeForLogging(error);
    ...
    it('should handle primitive types', () => {
      expect(cryptoService.security.sanitizeForLogging('string')).toBe('string');
      expect(cryptoService.security.sanitizeForLogging(123)).toBe(123);
      expect(cryptoService.security.sanitizeForLogging(true)).toBe(true);
      expect(cryptoService.security.sanitizeForLogging(null)).toBe(null);
      expect(cryptoService.security.sanitizeForLogging(undefined)).toBe(undefined);
    });
    ...
    it('should redact various sensitive key patterns', () => {
      ...
      const sanitized = cryptoService.security.sanitizeForLogging(data);
  ...
  describe('Input Validation and Sanitization', () => {
    it('should normalize keys by removing non-hex characters', () => {
      const dirtyKey = 'abc123XYZ!@#$%^&*()def456';
      const normalized = cryptoService.normalizeKey(dirtyKey);
    ...
    it('should reject keys with incorrect length after normalization', () => {
      const shortKey = 'abc123';
      const normalized = cryptoService.normalizeKey(shortKey);
      
      // Should return empty string for invalid length
      expect(normalized).toBe('');
    });

    it('should validate public keys strictly', () => {
  ...
  describe('Integration: Security in Crypto Operations', () => {
    ...
    it('should log security events for invalid signatures', async () => {
      ...
      // Mock to return false for this specific case
      const { verifyNostrEventSignature } = await import('@dweb/nostr/verify-nostr-event-signature');
      vi.mocked(verifyNostrEventSignature).mockResolvedValueOnce(false);
