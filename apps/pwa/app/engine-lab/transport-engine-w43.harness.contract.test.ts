...
const networkProtocolHostResult = {
  success: false,
  successCount: 0,
  totalRelays: 2,
  quorumRequired: 1,
  metQuorum: false,
  results: [
    { relayUrl: "wss://relay.one", success: false, error: "No writable relay connection" },
    { relayUrl: "wss://relay.two", success: false, error: "No writable relay connection" },
  ],
  failures: [
    { relayUrl: "wss://relay.one", success: false, error: "No writable relay connection" },
    { relayUrl: "wss://relay.two", success: false, error: "No writable relay connection" },
...
describe("transport-engine w43 — network protocol publish parity harness", () => {
  beforeEach(() => {
    mockInvoke.mockReset();
    resetTransportEngineHostForTests();
  });

  it("accepts protocol-backed network publish results through typed host adapter", async () => {
    mockInvoke.mockResolvedValue({ ok: true, data: networkProtocolHostResult });
    ...
    if (!result.ok) {
      throw new Error("Expected protocol network publish result.");
    }
    expect(isTransportPublishRelayEventResult(result.data)).toBe(true);
  });

  it("aligns protocol network publish quorum failure with shared mapper semantics", async () => {
    mockInvoke.mockResolvedValue({ ok: true, data: networkProtocolHostResult });
