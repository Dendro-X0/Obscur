...
export default function PrivacySettingsTabPanel(): React.JSX.Element {
    ...
    const toggleRowClass = cn("flex items-center justify-between gap-4", compact ? "py-1" : "rounded-2xl border border-black/5 bg-zinc-50/50 p-4 dark:border-white/10 dark:bg-zinc-900/40");
    return (<>
          <div className={compact ? "space-y-4" : "space-y-6"}>
            <div id="privacy-trust-settings">
              <TrustSettingsPanel />
            </div>
            <SettingsCompactCard title={t("settings.privacy.global")} description={t("settings.privacy.globalDesc")} className="w-full">
              <div className={compact ? "space-y-3" : "space-y-5"}>
                <SettingsCompactSection title={t("settings.privacy.dmPolicyTitle")} hint={compact ? undefined : t("settings.privacy.dmPolicyHint")}>
                  <div className={cn("grid gap-2", compact ? "grid-cols-2" : "sm:grid-cols-2")}>
                    <Button type="button" variant={privacySettings.dmPrivacy === "everyone" ? "secondary" : "outline"} onClick={() => handleSavePrivacy({ ...privacySettings, dmPrivacy: "everyone" })} className={cn("justify-start", compact && "h-9 text-xs")}>
                      {t("settings.privacy.everyone")}
                    </Button>
                    <Button type="button" variant={privacySettings.dmPrivacy === "contacts-only" ? "secondary" : "outline"} onClick={() => handleSavePrivacy({ ...privacySettings, dmPrivacy: "contacts-only" })} className={cn("justify-start", compact && "h-9 text-xs")}>
                      {t("settings.privacy.contactsOnly")}
                    </Button>
                  </div>
                ...
                <div className={toggleRowClass}>
                  <div className="min-w-0 flex-1">
                    <Label className="text-sm font-semibold tracking-wide">{t("settings.privacy.modernDmsTitle")}</Label>
                    {!compact ? (<p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{t("settings.privacy.modernDmsDesc")}</p>) : null}
                  </div>
                ...
                <SettingsCompactSection title={t("settings.privacy.retentionTitle")} hint={compact ? undefined : t("settings.privacy.retentionHint")}>
                  <div className={cn("grid gap-2", compact ? "grid-cols-3" : "sm:grid-cols-3")}>
                    {([
            { labelKey: "settings.privacy.retentionOff", days: 0 as const },
            { labelKey: compact ? "settings.privacy.retention30DaysShort" : "settings.privacy.retention30Days", days: 30 as const },
            { labelKey: compact ? "settings.privacy.retention90DaysShort" : "settings.privacy.retention90Days", days: 90 as const },
                ...
                <div className={toggleRowClass}>
                  <div className="min-w-0 flex-1">
                    <Label className="text-sm font-semibold tracking-wide">{t("settings.privacy.showPubkeyTitle")}</Label>
                    {!compact ? (<p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                        {t("settings.privacy.showPubkeyDesc")}
                      </p>) : null}
                  </div>
                ...
                {!compact ? (<SettingsActionStatus title={t("settings.privacy.summaryTitle")} phase="idle" summary={t("settings.privacy.summary", {
                dmPolicy: privacySettings.dmPrivacy === "everyone"
                    ? t("settings.privacy.dmPolicyEveryone")
                    : t("settings.privacy.dmPolicyContactsOnly"),
                modernDms: privacySettings.useModernDMs
                    ? t("settings.privacy.summaryModernDmsEnabled")
                    : t("settings.privacy.summaryModernDmsDisabled"),
                retention: `${privacySettings.localMessageRetentionDays || 0}d`,
                pubkeyControls: privacySettings.showPublicKeyControlsInChat
                    ? t("settings.privacy.summaryPubkeyShown")
                    : t("settings.privacy.summaryPubkeyHidden"),
            })}/>) : null}
              </div>
            </SettingsCompactCard>
          </div>
