...
export default function BlocklistSettingsTabPanel(): React.JSX.Element {
    ...
    return (<>
        <SettingsCompactCard title={t("settings.tabs.blocklist")} description={t("settings.blocklist.desc")} className="w-full">
          <div className={compact ? "space-y-4" : "space-y-6"}>
            {!compact ? (<div className="group relative overflow-hidden rounded-[2rem] border border-black/10 bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-6 shadow-sm transition-all hover:shadow-md dark:border-white/10 dark:from-zinc-900 dark:via-zinc-900 dark:to-zinc-950">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.25em] text-purple-600/60 dark:text-purple-400/60">
                      {t("settings.blocklist.hero.kicker")}
                    </span>
                  </div>
                  <h4 className="text-xl font-black text-zinc-950 dark:text-zinc-100 tracking-tight">
                    {t("settings.blocklist.hero.title")}
                  </h4>
                  <p className="max-w-md text-xs font-bold leading-relaxed text-zinc-500 dark:text-zinc-400">
                    {t("settings.blocklist.hero.desc")}
                  </p>
                </div>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3">
                ...
                <div className="flex flex-1 items-center justify-between rounded-xl border border-black/5 bg-zinc-50/80 px-3 py-2 text-xs dark:border-white/10 dark:bg-zinc-900/50">
                  <span className="font-semibold text-zinc-500">{t("settings.blocklist.compact.blocked")}</span>
                  <span className="font-black text-zinc-900 dark:text-zinc-100">{blocklist.state.blockedPublicKeys.length}</span>
                </div>
                <div className="flex flex-1 items-center justify-between rounded-xl border border-black/5 bg-zinc-50/80 px-3 py-2 text-xs dark:border-white/10 dark:bg-zinc-900/50">
                  <span className="font-semibold text-zinc-500">{t("settings.blocklist.compact.filtered")}</span>
                  <span className="font-black text-zinc-900 dark:text-zinc-100">{filteredBlockedKeys.length}</span>
                </div>
              </div>)}

            <SettingsCompactSection title={t("settings.blocklist.blockByPublicKeyTitle")}>
              <div className="flex flex-col gap-2 sm:flex-row">
                <Input value={blocklistInput} onChange={(e) => setBlocklistInput(e.target.value)} placeholder={t("settings.blocklist.publicKeyPlaceholder")} className="h-10 border-black/10 bg-white/80 font-mono text-xs dark:border-white/10 dark:bg-black/20" onKeyDown={(e) => {
            if (e.key === "Enter") {
                e.preventDefault();
                handleAddBlockedKey();
            }
        }}/>
                <Button type="button" onClick={handleAddBlockedKey} className={cn("h-10 font-bold text-white bg-gradient-primary border-none shadow-sm hover:shadow-md transition-all", compact ? "w-full sm:w-auto sm:px-6" : "px-8")}>
                  {t("settings.blocklist.blockAction")}
                </Button>
              </div>
            </SettingsCompactSection>

            <SettingsCompactSection title={`${t("settings.blocklist.blockedUsers")} (${filteredBlockedKeys.length})`} action={(<Button type="button" variant="ghost" size="sm" onClick={handleUnblockAll} className="text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20">
                  {t("settings.blocklist.unblockAll")}
                </Button>)}>
              <Input value={blocklistQuery} onChange={(e) => setBlocklistQuery(e.target.value)} placeholder={t("settings.blocklist.searchPlaceholder")} className={cn("h-9 border-black/10 bg-white/90 text-xs dark:border-white/10 dark:bg-black/20", compact ? "w-full" : "mb-3 h-8 w-[180px]")}/>
              {filteredBlockedKeys.length === 0 ? (<p className="rounded-xl border border-dashed border-black/10 p-4 text-center text-xs italic text-zinc-500 dark:border-white/10 dark:text-zinc-400">
                  {t("settings.blocklist.empty")}
                </p>) : (<div className="max-h-[320px] space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                  {filteredBlockedKeys.map((pubkey: string) => (<div key={pubkey} className="group flex items-center justify-between gap-2 rounded-xl border border-black/5 bg-zinc-50/80 p-2.5 shadow-sm transition-all hover:border-zinc-300 hover:bg-white dark:border-white/10 dark:bg-zinc-900/50 dark:hover:border-zinc-700 dark:hover:bg-zinc-900">
                    ...
                    try {
                        ...
                        setModerationActionMessage(t("settings.blocklist.pubkeyCopied"));
                    }
                    catch {
                        setModerationActionPhase("error");
                        setModerationActionMessage(t("settings.blocklist.clipboardUnavailable"));
                    }
                }}>
                          {t("common.copy")}
                        </Button>
                    ...
                    setModerationActionPhase("success");
                    setModerationActionMessage(t("settings.blocklist.userUnblocked"));
                }}>
            ...
            {!compact ? (
              <SettingsActionStatus
                title={t("settings.blocklist.moderationActionsTitle")}
                phase={moderationActionPhase}
                message={moderationActionMessage || undefined}
                summary={t("settings.blocklist.moderationActionsSummary", {
                  blocked: blocklist.state.blockedPublicKeys.length,
