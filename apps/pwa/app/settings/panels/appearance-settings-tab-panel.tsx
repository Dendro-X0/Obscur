...
export default function AppearanceSettingsTabPanel(): React.JSX.Element {
    ...
    return (<>
        <SettingsCompactCard title={t("settings.appearance.title")} description={t("settings.appearance.desc")} className="w-full">
          <div className={compact ? "space-y-4" : "space-y-6"}>
            <SettingsCompactSection title={t("settings.language")} hint={compact ? undefined : `${t("settings.appearance.currentLanguage")}: ${i18n.language}`} action={(<Button type="button" variant="ghost" size="sm" onClick={() => void handleResetLanguage()} className={resetButtonClass}>
            ...
            <SettingsCompactSection title={t("settings.appearance.theme")} hint={compact ? undefined : `${t("settings.appearance.currentTheme")}: ${theme.preference}`} action={(<Button type="button" variant="ghost" size="sm" onClick={handleResetTheme} className={resetButtonClass}>
            ...
            <SettingsCompactSection title={t("settings.appearance.accessibility")} hint={compact ? undefined : `${t("settings.appearance.textScale")}: ${accessibility.preferences.textScale}%`} action={(<Button type="button" variant="ghost" size="sm" onClick={handleResetAccessibility} className={resetButtonClass}>
                  {t("settings.appearance.resetAccessibility")}
                </Button>)}>
              <div className="space-y-4">
                <div className={cn(compact ? "grid grid-cols-4 gap-1.5" : "flex flex-wrap gap-2")}>
                  {TEXT_SCALE_OPTIONS.map((scale: TextScale) => (<Button key={scale} type="button" size="sm" variant={accessibility.preferences.textScale === scale ? "primary" : "outline"} className={cn("font-black transition-all", compact ? "h-9 px-0 text-xs" : "h-10 px-4", accessibility.preferences.textScale === scale
            ...
            : "rounded-xl border border-black/5 bg-black/5 p-4 dark:border-white/5 dark:bg-black/20")}>
                  <div className="flex min-w-0 flex-col gap-1">
                    ...
                    {!compact ? (<div className="text-xs text-zinc-500 dark:text-zinc-400">
            ...
            {(!compact || appearanceActionPhase !== "idle" || appearanceActionMessage) ? (<SettingsActionStatus title={t("settings.appearance.statusTitle")} phase={appearanceActionPhase} message={appearanceActionMessage || undefined} summary={t("settings.appearance.statusSummary")}/>) : null}
          </div>
        </SettingsCompactCard>
