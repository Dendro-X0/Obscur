...
export default function RelaysSettingsTabPanel(): React.JSX.Element {
  ...
  const relayScopePartition = useMemo(
    () => partitionRelayListByTransportScope(relayList.state.relays),
    [relayList.state.relays],
  );

  const renderRelayListSection = (
    sectionRelays: ReadonlyArray<{ url: string; enabled: boolean }>,
    options: Readonly<{ dmTransport: boolean }>,
  ): React.ReactNode => (
        ...
        >
          ...
          <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all scale-95 group-hover:scale-100">
            ...
            <Button
              ...
              disabled={index === sectionRelays.length - 1}
            >
    ...
    <>
        <Card title={t("settings.relays.title")} description={t("settings.relays.desc")} className="w-full">
          <div className="space-y-6">
            ...
            <div
              id="relay-api-status"
              className="space-y-4 rounded-2xl border border-black/5 p-5 dark:border-white/5 bg-zinc-50/50 dark:bg-zinc-900/50"
            >
            ...
            <div className="space-y-4 rounded-2xl border border-black/5 p-5 dark:border-white/5 bg-white dark:bg-black/20">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-zinc-500">
                    {t("settings.relays.scopeSummary", {
                      defaultValue:
                        "DM transport: {{dmEnabled}} enabled · Workspace candidates: {{communityEnabled}} enabled ({{communityConfigured}} configured). Unreachable intranet relays stay out of the DM pool until you enable them here.",
                  ...
                  <p className="text-xs font-medium text-zinc-600 dark:text-zinc-300">
                    {getActiveTransportScopeCopy({
                      ...
                      enabledRelayCount: relayList.state.relays.filter((relay: { enabled: boolean }) => relay.enabled).length,
                    })}
                  </p>
                </div>
                <Button
                  ...
                  onClick={() => setShowAdvancedRelays((prev: boolean) => !prev)}
                >
                  {showAdvancedRelays
              ...
              <div className="rounded-xl border border-black/5 bg-zinc-50 p-4 dark:border-white/10 dark:bg-zinc-900/50">
                <div className="flex flex-wrap items-center gap-3 text-xs">
                  <span className={cn(
                    ...
                    {relayRuntimeStatus.status === "healthy"
                      ? t("settings.relays.statusPrimary", "Active transport")
                      : relayRuntimeStatus.status === "recovering"
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {RELAY_PRESETS.map((preset: { id: string; label: string }) => (
              ...
              <div
                ...
                className="rounded-2xl border border-black/5 bg-zinc-50/70 p-5 dark:border-white/10 dark:bg-zinc-900/40"
              >
                ...
                >
                  <div className="space-y-4 rounded-2xl border border-black/5 p-5 dark:border-white/5 bg-zinc-50/50 dark:bg-zinc-900/50">
                    ...
                    <div className="space-y-4 pt-2">
                      <div className="space-y-2">
                        <Label className="text-xs font-semibold uppercase tracking-wider text-zinc-500">
                          {t("settings.relays.dmSectionTitle", "DM & Nostr transport")}
                        </Label>
                        <p className="text-[11px] leading-relaxed text-zinc-500">
                          {t(
                            "settings.relays.dmSectionDesc",
                            "Used for direct messages, profile publish, and the global active transport pool. Public Nostr relays belong here.",
                          )}
                        </p>
                        <div className="space-y-3">
                          {relayScopePartition.dm.length > 0
                            ...
                            : (
                              <p className="text-xs text-zinc-500">
                                {t("settings.relays.dmSectionEmpty", "No DM relays configured. Enable a public wss:// relay above.")}
                              </p>
                            )}
                        </div>
                      </div>
                      <div className="space-y-2 border-t border-black/5 pt-4 dark:border-white/10">
                        <Label className="text-xs font-semibold uppercase tracking-wider text-zinc-500">
                          {t("settings.relays.communitySectionTitle", "Workspace relay candidates")}
                        </Label>
                        <p className="text-[11px] leading-relaxed text-zinc-500">
                          {t(
                            "settings.relays.communitySectionDesc",
                            "Private or team relays for managed workspace communities. Each community binds one relay at create time; membership lists use coordination, not this DM pool.",
                          )}
                        </p>
                        <div className="space-y-3">
                          {relayScopePartition.community.length > 0
                            ...
                            : (
                              <p className="text-xs text-zinc-500">
                                {t(
                                  "settings.relays.communitySectionEmpty",
                                  "Add a private wss:// or ws://localhost relay for workspace communities. Disabled intranet nodes do not affect DM transport.",
                                )}
                              </p>
                            )}
                        </div>
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <RelaySettingsMetricsToolbar
                      category={relayMetricsCategory}
                      onCategoryChange={setRelayMetricsCategory}
                      availableOnly={relayMetricsAvailableOnly}
                      onAvailableOnlyChange={setRelayMetricsAvailableOnly}
                    />
