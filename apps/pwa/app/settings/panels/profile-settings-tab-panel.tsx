...
export default function ProfileSettingsTabPanel(): React.JSX.Element {
    ...
    return (<>
        <div className={compact ? "space-y-3" : "space-y-4"}>
          <SettingsCompactCard title={t("profile.title")} description={t("profile.description")} className="w-full">
            <div id="profile" className={compact ? "space-y-4" : "space-y-6"}>
              <div className={cn("space-y-6 rounded-2xl border border-black/10 bg-gradient-to-br from-white/90 to-zinc-50/50 backdrop-blur-md shadow-sm dark:border-white/10 dark:from-zinc-900/40 dark:to-zinc-950/20", compact ? "space-y-4 p-4" : "p-6")}>
                ...
                <div className="space-y-2">
                  ...
                  {!compact ? (<div className="text-xs text-zinc-600 dark:text-zinc-400">{t("profile.usernameHelp")}</div>) : null}
                ...
                <div className="space-y-2">
                  ...
                  <div className={cn("flex items-center justify-between text-xs text-zinc-600 dark:text-zinc-400", compact && "justify-end")}>
                    {!compact ? (<span>{t("profile.aboutHelp")}</span>) : null}
                ...
                <div className="space-y-2">
                  ...
                  <div className={cn("flex gap-2", compact && "flex-col")}>
                    ...
                    <Button type="button" variant="secondary" className={compact ? "w-full" : undefined} onClick={handleVerifyNip05} disabled={isVerifyingNip05 || !!profileValidation.nip05Error}>
                      {isVerifyingNip05 ? <Loader2 className="h-4 w-4 animate-spin"/> : t("profile.verifyNip05")}
                    </Button>
                  </div>
                  {!compact ? (<div className="text-xs text-zinc-600 dark:text-zinc-400">{t("profile.nip05Help")}</div>) : null}
                ...
                <div className="space-y-2">
                  <Label htmlFor="profile-invite-code">{t("profile.friendCodeLabel")}</Label>
            ...
            try {
                ...
                toast.success(t("profile.friendCodeCopied"));
            }
            catch {
                toast.error(t("profile.friendCodeCopyFailed"));
            }
        }}>
                      {t("profile.friendCodeCopy")}
                    </Button>
                  </div>
                  <div className={cn("flex flex-wrap items-center gap-2 text-xs text-zinc-600 dark:text-zinc-400", compact && "flex-col items-start")}>
                    {!compact ? (<>
                        <span>{t("profile.friendCodeHelp")}</span>
                        <span>{t("profile.friendCodeSuffixHelp")}</span>
                      </>) : (<span>{t("profile.friendCodeSuffixHelpCompact")}</span>)}
                ...
                toast.success(t("profile.identityLinkCopied"));
            }}>
                        {t("profile.copyIdentityLink")}
                      </Button>) : null}
                  </div>
              ...
              <div className={cn("flex flex-col gap-3", compact ? "pt-2" : "pt-4 sm:flex-row")}>
                <Button type="button" disabled={isPublishing || !profileValidation.isValid || inviteCodeAvailabilityStatus === "checking"} className={cn("h-11 font-bold text-white bg-gradient-primary border-none shadow-md hover:shadow-lg transition-all active:scale-[0.98]", compact ? "w-full px-4" : "px-8")} onClick={handleSaveProfile}>
              ...
              {(!compact || !profileSaveIdle) ? (<SettingsActionStatus title="Profile Save Status" phase={profileSavePhase} message={profilePreflightError
                ...
                : accountSyncSnapshot.portabilityStatus === "portable"
                    ...
                    : accountSyncSnapshot.portabilityStatus === "profile_only"
                        ...
                        : accountSyncSnapshot.portabilityStatus === "local_only"
                            ...
                            : "Profile save and account sync are separate proof steps. Portability is only claimed after both relay-backed proofs succeed."}/>) : null}
              <div id="account-sync-backup" className={cn("rounded-2xl border border-black/10 bg-white/70 shadow-sm dark:border-white/10 dark:bg-zinc-900/50", compact ? "p-3" : "p-4")}>
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {!compact ? (<div className="text-xs text-zinc-600 dark:text-zinc-400">
            ...
            : "No relay proof yet"}
                    ...
                    {accountSyncSnapshot.latestProfileEventId && !compact ? (<div className="mt-1 font-mono text-[10px] text-zinc-500">{accountSyncSnapshot.latestProfileEventId.slice(0, 16)}...</div>) : null}
                  </div>
                  <div className="rounded-xl bg-zinc-50 p-3 text-xs dark:bg-zinc-950/60">
            ...
            : "No relay proof yet"}
                    ...
                    {accountSyncSnapshot.latestBackupEventId && !compact ? (<div className="mt-1 font-mono text-[10px] text-zinc-500">{accountSyncSnapshot.latestBackupEventId.slice(0, 16)}...</div>) : null}
                  </div>
                </div>
                {!compact ? (<WorkspaceKernelBackupRestoreScopeNotice className="mt-3"/>) : (<WorkspaceKernelBackupRestoreScopeNotice compact className="mt-3"/>)}
                ...
                {!compact ? (<div className="mt-3 rounded-xl border border-black/10 bg-zinc-50 px-3 py-2 text-xs text-zinc-600 dark:border-white/10 dark:bg-zinc-950/60 dark:text-zinc-400">
                  Last restore source: {accountSyncSnapshot.lastRestoreSource?.replace("_", " ") || "none"}
                </div>) : null}
                <PortabilityQuickActionsPanel compact className={compact ? "mt-4" : "mt-8"} publicKeyHex={publicKeyHex} profileLabel={profile.state.profile.username.trim() || undefined} resolveActivePrivateKeyHex={resolveActivePrivateKeyHex}/>
                <ManualPortabilityPanel publicKeyHex={publicKeyHex} profileLabel={profile.state.profile.username.trim() || undefined} resolveActivePrivateKeyHex={resolveActivePrivateKeyHex}/>
              </div>
            </div>
          </SettingsCompactCard>
        </div>
