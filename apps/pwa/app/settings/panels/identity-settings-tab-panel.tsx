    ...
    return (<>
        <div className={compact ? "space-y-4" : "space-y-6"}>
          ...
          <SettingsCompactCard title={t("identity.title")} description={t("identity.description")} className="w-full">
            <div className={compact ? "space-y-4" : "space-y-6"}>
              <div className={cn("relative overflow-hidden rounded-2xl border border-black/10 bg-gradient-to-br from-zinc-50 to-white shadow-sm dark:border-white/10 dark:from-zinc-900/40 dark:to-zinc-950/20", compact ? "p-4" : "p-5")}>
              ...
              <div className="space-y-4">
                ...
                <AnimatePresence mode="wait">
                  {!isPrivateKeyVisible && !isChallenging ? (<motion.div key="locked" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                      <Button variant="outline" className={cn("w-full rounded-2xl border-2 border-dashed border-black/10 bg-transparent hover:bg-purple-500/5 hover:border-purple-500/40 group transition-all dark:border-white/10 dark:hover:bg-purple-500/10", compact ? "h-12" : "h-16")} onClick={handleRevealToggle} disabled={identityIntegrityState === "mismatch"}>
            ...
            <PortabilityQuickActionsPanel compact className="mt-6" publicKeyHex={(publicKeyHex as PublicKeyHex | null) ?? null} profileLabel={profile.state.profile.username} resolveActivePrivateKeyHex={resolveActivePrivateKeyHex}/>

            <div id="local-data-management" className={cn("mt-8 rounded-xl border border-red-200 bg-red-50 dark:border-red-900/30 dark:bg-red-950/10 space-y-3", compact ? "p-3" : "p-4")}>
