...
import { getLocalMediaStorageConfig, getLocalMediaStorageAbsolutePath, saveLocalMediaStorageConfig, DEFAULT_LOCAL_MEDIA_STORAGE_CONFIG, type LocalMediaStorageConfig, } from "@/app/features/vault/services/local-media-store";
...
import { deriveStorageMode, deriveStorageStats, type StorageMode, type StorageStats, } from "../settings-tab-panel-shared";
...
export function useStorageSettingsModel(): SettingsTabPanelModel {
    ...
    const storageMode = useMemo<StorageMode>(() => deriveStorageMode(nip96Config.enabled, localMediaConfig.enabled), [localMediaConfig.enabled, nip96Config.enabled]);
