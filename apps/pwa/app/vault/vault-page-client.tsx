...
export default function VaultPageClient(): React.JSX.Element {
    ...
    return (
        <PageShell title={t("nav.vault", "Vault")} navBadgeCounts={navBadges.navBadgeCounts} containScroll={compact}>
            <div className={cn(
                "mx-auto flex w-full min-h-0 flex-1 flex-col overflow-hidden",
                compact ? "max-w-3xl p-3" : tablet ? "max-w-4xl p-4" : "max-w-7xl p-6 py-8",
            )}>
                <div className={cn(
                    "flex shrink-0 flex-col items-start justify-between",
                    compact ? "mb-4 gap-3" : "mb-12 gap-6 md:flex-row md:items-center",
                )}>
                    <div className={cn("flex items-center", compact ? "gap-3" : "gap-6")}>
                        <div className={cn(
                            "flex shrink-0 items-center justify-center rounded-[28px] bg-gradient-to-br from-primary to-indigo-600 shadow-2xl shadow-primary/20",
                            compact ? "h-12 w-12 rounded-2xl" : "h-20 w-20",
                        )}>
                            <Lock className={cn("text-white", compact ? "h-5 w-5" : "h-10 w-10")} />
                        </div>
                        <div className="space-y-0.5">
                            <h2 className={cn(
                                "font-black tracking-tight text-zinc-900 dark:text-white",
                                compact ? "text-xl" : "text-3xl",
                            )}>{t("nav.vault", "The Vault")}</h2>
                            {!compact ? (
                    ...
                    <div className={cn("flex items-center gap-2", compact ? "w-full" : "w-full gap-3 md:w-auto")}>
                        <Button
                            ...
                            className={cn(
                                "rounded-2xl bg-primary font-black text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:bg-primary/90",
                                compact ? "h-11 min-h-[44px] flex-1 px-4 text-sm" : "h-12 flex-1 px-6 md:flex-none",
                            )}
                        >
                ...
                <div className={cn("flex min-h-0 flex-1 flex-col", compact ? "space-y-4" : "space-y-8")}>
                    <div className="flex shrink-0 items-center gap-2">
                        ...
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">{t("vault.recentMedia", "Recent Media")}</h3>
                    </div>
                    <div
                        data-testid="vault-mobile-scroll-region"
                        className={cn(
                            "min-h-0 flex-1",
                            compact && "mobile-scroll-region overflow-y-auto",
                        )}
                    >
                        <VaultMediaGrid
