...
export function SearchResultCard({ result, targetElementId, onClick, onAdd }: SearchResultCardProps) {
    ...
    const resolvedTitle = resolvedMetadata.displayName || result.display.title || result.display.pubkey || t("search.discovery.identity.unknownContact");
    ...
    return (<div id={targetElementId} role="button" tabIndex={0} onClick={handleClick} onKeyDown={(e) => {
            if (e.key === 'Enter')
                handleClick();
            ...
            <div className="relative">
                <Avatar className="h-12 w-12 border border-border/70 bg-muted shadow-sm transition-colors group-hover:border-primary/40 sm:h-14 sm:w-14">
                    {resolvedPicture ? (<Image src={resolvedPicture} alt={resolvedTitle} width={56} height={56} className="object-cover"/>) : (<AvatarFallback className="bg-muted">
                            {getIcon()}
                        </AvatarFallback>)}
                </Avatar>

                {(result.kind === "person" || result.kind === "invite" || result.kind === "contact_card") && (<div className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full border-2 border-background bg-primary opacity-0 transition-opacity group-hover:opacity-100">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    ...
                    {result.confidence === "direct" && (<span className="rounded-full bg-primary/8 px-2 py-0.5 text-[10px] font-black uppercase tracking-widest text-primary/70">
                            {t("search.discovery.badge.direct")}
                        </span>)}
                </div>

                {resolvedSubtitle && resolvedSubtitle !== resolvedTitle && (<p className="truncate text-sm font-medium text-muted-foreground/80">
                ...
                {resolvedDescription && (<p className="mt-2 line-clamp-2 text-xs leading-relaxed text-muted-foreground/75">
                        {resolvedDescription}
                    </p>)}
            </div>
            ...
            <div className="flex translate-x-0 items-center gap-2 opacity-100 transition-all duration-300 sm:translate-x-2 sm:opacity-0 sm:group-hover:translate-x-0 sm:group-hover:opacity-100">
                {(result.kind === "person" || result.kind === "invite" || result.kind === "contact_card") && (<Button size="icon" variant="ghost" onClick={handleAdd} className="h-8 w-8 rounded-xl bg-primary/10 text-primary transition-all hover:bg-primary hover:text-primary-foreground sm:h-9 sm:w-9">
            ...
            <div className="absolute right-4 top-2">
                <span className={cn("rounded-full px-2 py-0.5 text-[8px] font-black uppercase tracking-[0.2em]", result.kind === "community" ? "bg-amber-500/10 text-amber-600 dark:text-amber-300" : "bg-indigo-500/10 text-indigo-600 dark:text-indigo-300")}>
                    {result.kind === "community"
            ? t("search.discovery.badge.community")
            : t("search.discovery.badge.profile")}
                </span>
            </div>
        </div>);
}
