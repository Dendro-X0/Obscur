...
import { SettingsToggleCard } from "@/app/settings/settings-tab-panel-model";
import {
  RELAY_SETTINGS_CATEGORY_ORDER,
  type RelaySettingsCategory,
  ...
  return (
    <div className="space-y-3 px-1">
      <div
        ...
        role="tablist"
        aria-label={t("settings.relays.categoryTabs", "Relay categories")}
      >
        {RELAY_SETTINGS_CATEGORY_ORDER.map((tab) => {
          const isActive = params.category === tab;
          const label = tab === "all"
            ? t("settings.relays.categoryAll", "All")
          ...
          return (
            <button
              ...
              onClick={() => params.onCategoryChange(tab)}
              className={cn(
      ...
      <SettingsToggleCard
        title={t("settings.relays.availableOnlyTitle", "Available nodes only")}
        description={t(
          "settings.relays.availableOnlyDesc",
          "Show relays that are connected, connecting, or recently probed — hide disabled and disconnected nodes.",
        )}
        checked={params.availableOnly}
        onChange={params.onAvailableOnlyChange}
      />
    </div>
  );
}
