...
} from "@/app/features/groups/services/community-coordination-health";
import {
...
import {
    readAssumeLocalCoordinationReachable,
...
} from "@/app/features/groups/services/community-dev-flags";
...
export function OperatorTrustSetupWizard(): React.JSX.Element {
    ...
    const [assumeLocalCoordination, setAssumeLocalCoordination] = React.useState(
        () => readAssumeLocalCoordinationReachable(),
    );
    ...
    const handleApply = async (): Promise<void> => {
        ...
        if (!coord) {
            setPhase("error");
            setStatusMessage(t(
                "settings.operatorSetup.coordinationRequired",
                "Enter a coordination service URL (e.g. http://127.0.0.1:8787).",
            ));
            return;
        }
        if (!testWithoutRelay && !rawRelayInput) {
            ...
            setStatusMessage(t(
                ...
                "Enter a private workspace relay URL (e.g. ws://localhost:7000).",
            ));
            return;
        }
        ...
        if (!health.healthy && !assumeLocalCoordination) {
            setPhase("error");
            setStatusMessage(t(
                "settings.operatorSetup.coordinationUnreachable",
                "Coordination /health did not return ok. Run: pnpm -C apps/coordination dev",
            ));
            return;
        }
        ...
        setStatusMessage(
            testWithoutRelay
                ? t(
                    "settings.operatorSetup.successNoRelay",
                    "Coordination-only dev enabled. Create groups without Docker or a live Nostr relay; chat publish stays local until you add a relay later.",
                )
                : t(
                    "settings.operatorSetup.success",
                    "Operator bundle applied: coordination directory + workspace relay. You can create managed workspace communities.",
                ),
        );
        toast.success(t("settings.operatorSetup.successToast", "Private trust stack configured."));
    };

    return (
        <div
            id="operator-trust-setup"
            className="space-y-4 rounded-2xl border border-emerald-500/25 bg-emerald-500/5 p-5 dark:border-emerald-500/20 dark:bg-emerald-950/20"
            data-testid="operator-trust-setup-wizard"
        >
            <div className="space-y-1">
                <div className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-emerald-800 dark:text-emerald-200">
                    ...
                    {t("settings.operatorSetup.title", "Operator setup (private trust)")}
                </div>
                <p className="text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
                    {t(
                        "settings.operatorSetup.desc",
                        "Bundle coordination membership directory and a private workspace relay. DM may still use public Nostr relays; workspace communities require this stack.",
                    )}
                </p>
                {buildEnvUrl ? (
                    <p className="text-[10px] text-zinc-500">
                        {t("settings.operatorSetup.buildEnv", "Build default:")}
                        {" "}
                        ...
                        {coordinationSource === "runtime_override" ? (
                            <span className="ml-2 font-semibold text-emerald-700 dark:text-emerald-300">
                                ({t("settings.operatorSetup.usingOverride", "using device override")})
                            </span>
                        ) : null}
                    </p>
                ) : null}
            ...
            <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">
                        ...
                        {t("settings.operatorSetup.coordinationLabel", "Coordination URL")}
                    </Label>
                    <Input
                        value={coordinationUrl}
                        onChange={(event) => setCoordinationUrl(event.target.value)}
                        placeholder="http://127.0.0.1:8787"
                        className="font-mono text-xs"
                        data-testid="operator-setup-coordination-url"
                    />
                </div>
                <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-500">
                        <Wifi className="mr-1 inline h-3 w-3" />
                        {t("settings.operatorSetup.relayLabel", "Workspace relay")}
                    </Label>
                    <Input
                        value={workspaceRelayUrl}
                        onChange={(event) => setWorkspaceRelayUrl(event.target.value)}
                        placeholder="ws://localhost:7000"
                        className="font-mono text-xs"
                        data-testid="operator-setup-workspace-relay"
                    />
                </div>
            </div>
            ...
            {statusMessage ? (
                <p
                    ...
                    data-testid="operator-setup-status"
                >
                    {statusMessage}
                </p>
            ) : null}
            ...
            <label className="flex items-start gap-3 rounded-xl border border-zinc-200/80 bg-white/50 p-3 dark:border-zinc-800 dark:bg-black/20">
                <input
                    type="checkbox"
                    className="mt-1"
                    checked={assumeLocalCoordination}
                    onChange={(event) => setAssumeLocalCoordination(event.target.checked)}
                    data-testid="operator-setup-assume-coordination"
                />
                <span className="text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
                    <span className="font-semibold text-zinc-900 dark:text-white">
                        {t("settings.operatorSetup.assumeCoordinationTitle", "Trust local coordination (curl verified)")}
                    </span>
                    <br />
                    {t(
                        "settings.operatorSetup.assumeCoordinationDesc",
                        "Use when terminal curl http://127.0.0.1:8787/health works but the desktop app probe fails. Unblocks create UI; membership publish still needs a working path to coordination.",
                    )}
                </span>
            </label>
            ...
            <label className="flex items-start gap-3 rounded-xl border border-zinc-200/80 bg-white/50 p-3 dark:border-zinc-800 dark:bg-black/20">
                ...
                <span className="text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
                    <span className="font-semibold text-zinc-900 dark:text-white">
                        {t("settings.operatorSetup.skipRelayTitle", "Test without local Nostr relay")}
                    </span>
                    <br />
                    {t(
                        "settings.operatorSetup.skipRelayDesc",
                        "No VPS or Docker required. Membership uses coordination only; encrypted community chat stays on-device until you run pnpm dev:relay later.",
                    )}
                </span>
            </label>
            ...
            <div className="flex flex-wrap gap-2">
                <Button
                    ...
                    className={cn(phase === "probing" && "!opacity-100 cursor-wait")}
                    data-testid="operator-setup-apply"
                >
                    {phase === "probing" ? (
                        <ActionButtonSpinner className="mr-2 border-white/35 border-t-white text-white" />
                    ) : (
                        <Check className="mr-2 h-4 w-4" />
                    )}
                    {phase === "probing"
                        ? t("settings.operatorSetup.applying", "Applying…")
                        : t("settings.operatorSetup.apply", "Apply operator bundle")}
                </Button>
                <Button
                    type="button"
                    variant="ghost"
                ...
                >
                    {t("settings.operatorSetup.resetOverrides", "Clear device overrides")}
                </Button>
            </div>

            {readOperatorCoordinationUrlOverride() ? (
                <p className="text-[10px] text-zinc-500 font-mono">
                    {t("settings.operatorSetup.overrideActive", "Device override:")}
                    {" "}
                    {readOperatorCoordinationUrlOverride()}
                </p>
            ) : null}
