...
export const EmptyConversationView: React.FC<EmptyConversationViewProps> = ({ showWelcome, myPublicKeyHex, relayStatus, onCopyMyPubkey, onCopyChatLink, onNewChat, showHistorySyncNotice = false, }) => {
    const { t } = useTranslation();
    return (<div className="flex flex-col flex-1 h-full w-full items-center justify-center p-8 bg-transparent">
            <div className="w-full max-w-lg flex flex-col items-center justify-center text-center relative gap-8">
                {showWelcome && (<div className="relative group animate-in fade-in zoom-in-95 duration-1000 ease-out w-full">
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-300 w-full flex flex-col items-center">
                    <div className={cn("w-full rounded-2xl border p-4 text-left", showHistorySyncNotice
            ? "border-indigo-500/20 bg-indigo-500/10"
            : "border-zinc-500/20 bg-zinc-500/10")}>
                        <div className={cn("flex items-center gap-2", showHistorySyncNotice
            ? "text-indigo-800 dark:text-indigo-200"
            : "text-zinc-700 dark:text-zinc-200")}>
                            {showHistorySyncNotice ? (<Loader2 className="h-4 w-4 animate-spin"/>) : (<Info className="h-4 w-4"/>)}
                            <span className="text-xs font-black uppercase tracking-widest">
                                {showHistorySyncNotice ? "Syncing account history" : "Missing contacts or chat history?"}
                            </span>
                        </div>
                        <p className={cn("mt-2 text-xs font-medium leading-relaxed", showHistorySyncNotice
            ? "text-indigo-900/90 dark:text-indigo-100/90"
            : "text-zinc-800/90 dark:text-zinc-100/90")}>
                            {t("messaging.historySyncWaitHint")}
                        </p>
                    </div>
                    <div className="flex justify-center">
                        <div className="group relative">
                            ...
                            <button type="button" className="relative z-10 flex h-20 w-20 items-center justify-center rounded-3xl border border-black/5 bg-white text-3xl dark:border-white/5 dark:bg-zinc-900/60 shadow-xl shadow-black/5 transition-transform hover:scale-105 active:scale-95 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500" onClick={onNewChat} aria-label="Start a new conversation">
                ...
                {myPublicKeyHex ? (<div className="animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500">
                        <Card className="w-full border-purple-500/10 bg-white/40 dark:bg-zinc-900/40 backdrop-blur-xl">
                            <div className="space-y-5 text-left">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <Button type="button" variant="secondary" onClick={onCopyMyPubkey} className="h-11 rounded-2xl group">
                                    ...
                                    <Button type="button" variant="secondary" onClick={onCopyChatLink} className="h-11 rounded-2xl group">
