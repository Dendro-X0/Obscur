/**
 * Web-only legacy relay recovery controller — pool adapter + watchdog orchestration.
 * Import only through relay-recovery-port.
 */
...
const createDefaultSnapshot = createDefaultRelayRecoverySnapshot;

const extractAdapterMetrics = extractRelayRecoveryAdapterMetrics;
...
class RelayRecoveryController {
  ...
  refreshSnapshot(): RelayRecoverySnapshot {
    ...
    const metrics = extractAdapterMetrics(this.config);
    const fallbackWritableRelayCount = metrics.fallbackWritableRelayCount;
    ...
    const nextSnapshot = buildTransportRecoverySnapshot({
      metrics,
      recoveryState: {
...
export const createWebLegacyRelayRecoveryController = (): RelayRecoveryController => (
  new RelayRecoveryController()
);

/** @deprecated Use createWebLegacyRelayRecoveryController via relay-recovery-port */
