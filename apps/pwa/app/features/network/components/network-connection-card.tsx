...
export const ConnectionCard = ({ pubkey, displayName, online = false, onClick, className, viewMode = "list" }: ConnectionCardProps) => {
    ...
    const statusLabel = online ? t("network.online") : t("network.offline");
    ...
    if (viewMode === "list") {
        return (<div onClick={onClick} className={cn("group flex cursor-pointer items-center justify-between rounded-xl border border-transparent px-3 py-3 transition-all hover:border-border/70 hover:bg-card/70", className)}>
                <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className="relative">
                        <UserAvatar pubkey={pubkey} size="lg" showProfileOnClick={false} className="bg-muted border border-border"/>
                        <div className={cn("absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-background", online ? "bg-emerald-500" : "bg-zinc-400 dark:bg-zinc-600")}/>
                    </div>
                ...
                <div className="flex items-center gap-4 shrink-0">
                    <span className={cn("hidden rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest sm:inline-block", statusTone)}>
                        {statusLabel}
                    </span>
    ...
    return (<div onClick={onClick} className={cn("group relative flex cursor-pointer flex-col items-center rounded-[28px] border border-border/70 bg-card/65 p-6 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-emerald-500/35 hover:shadow-2xl hover:shadow-emerald-950/10 active:scale-[0.98]", className)}>
            <div className="relative mb-4">
                <UserAvatar pubkey={pubkey} size="xl" showProfileOnClick={false} className="ring-4 ring-background shadow-xl group-hover:scale-110 transition-transform duration-500"/>
                <div className={cn("absolute -bottom-1 -right-1 h-5 w-5 rounded-full border-4 border-background shadow-sm", online ? "bg-emerald-500" : "bg-zinc-400 dark:bg-zinc-600")}/>
            </div>
            ...
            <div className="flex w-full items-center justify-center gap-2 border-t border-border/20 pt-2">
                <span className={cn("rounded-full border px-3 py-1 text-[9px] font-black uppercase tracking-widest", statusTone)}>
                    {statusLabel}
                </span>
