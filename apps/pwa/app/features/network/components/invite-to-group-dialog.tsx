...
export function InviteToGroupDialog({ isOpen, onClose, onInvite, targetPubkey }: InviteToGroupDialogProps) {
    ...
    const membershipIndex = useCommunityMembershipReadModelIndex({
        ...
        groups: React.useMemo(() => (createdGroups.map((group) => ({
            conversationId: group.id,
            groupId: group.groupId,
            relayUrl: group.relayUrl,
            directoryParticipantPubkeys: (communityKnownParticipantDirectoryByConversationId[group.id]?.participantPubkeys ?? []) as ReadonlyArray<PublicKeyHex>,
            persistedGroupMemberPubkeys: (group.memberPubkeys ?? []) as ReadonlyArray<PublicKeyHex>,
            projectionMemberPubkeys: (communityRosterByConversationId[group.id]?.activeMemberPubkeys ?? undefined) as ReadonlyArray<PublicKeyHex> | undefined,
    ...
    const filteredGroups = React.useMemo(() => {
        return createdGroups.filter(g => g.displayName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const resolveInviteMemberCount = (group: GroupConversation): number => (membershipIndex[group.id]?.memberCount ?? 1);
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="w-full max-w-md rounded-3xl border border-zinc-200/50 bg-white dark:border-white/10 dark:bg-[#121214] p-6 shadow-2xl">
                <div className="mb-5">
                    <div className="text-sm font-bold tracking-tight text-zinc-950 dark:text-zinc-50">{t("network.inviteTitle")}</div>
                    <div className="mt-1 text-xs font-medium leading-relaxed text-zinc-500 dark:text-zinc-400">{t("network.inviteDesc")}</div>
                </div>
                <div className="space-y-4">
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400"/>
                        <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t("groups.searchPlaceholder")} className="pl-9 bg-zinc-50 dark:bg-zinc-900/50 border-black/5 dark:border-white/5 focus:bg-white dark:focus:bg-zinc-900 transition-all rounded-xl"/>
                    </div>

                    <div className="max-h-64 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                        {filteredGroups.length === 0 ? (<div className="flex flex-col items-center justify-center py-8 text-center bg-zinc-50 dark:bg-white/5 rounded-2xl border border-dashed border-zinc-200 dark:border-white/10">
                                <Users className="h-8 w-8 text-zinc-300 dark:text-zinc-700 mb-2"/>
                                <p className="text-sm font-medium text-zinc-500">{t("groups.noGroupsFound")}</p>
            ...
            return (<button key={group.id} onClick={() => {
                ...
                }} disabled={alreadyMember} className={`
                    ...
                    : 'bg-purple-100 dark:bg-purple-900/30 group-hover:scale-105'}
                                        `}>
                                            {alreadyMember ? (<Check className="h-5 w-5 text-emerald-600 dark:text-emerald-400"/>) : (<Users className="h-5 w-5 text-purple-600 dark:text-purple-400"/>)}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="text-[10px] truncate font-mono opacity-60">
                                                {alreadyMember ? (<span className="text-emerald-600 dark:text-emerald-400">
                                                        {t("groups.alreadyMember")}
                                                    </span>) : (<span className="text-zinc-500">{group.groupId}</span>)}
                                            </div>
                                        </div>
                                    </button>);
        }))}
