...
export const CommunityInviteStatusBanner: React.FC<CommunityInviteStatusBannerProps> = ({
    ...
    const shellClass = isOutgoing
        ? cn(
            ...
            isAccepted && "border-amber-400/35 text-amber-100",
            isDeclined && "border-rose-400/30 text-rose-300",
            (isExpired || isSuperseded) && "border-zinc-400/35 text-zinc-300",
            !isAccepted && !isDeclined && !isExpired && !isSuperseded && "text-zinc-200",
        )
        : cn(
    ...
    const subtitleClass = isOutgoing
        ? cn(
            ...
            isAccepted && "text-amber-50/95",
            isDeclined && "text-rose-100/90",
            (isExpired || isSuperseded) && "text-zinc-300/90",
            !isAccepted && !isDeclined && !isExpired && !isSuperseded && "text-zinc-300/90",
        )
        : cn(
            "max-w-[28rem] text-[11px] font-medium leading-snug",
