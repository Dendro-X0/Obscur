...
import type { UseSealedCommunityResult } from "../hooks/use-sealed-community-types";
...
import { isManagedWorkspaceRelayGateBlocking, resolveManagedWorkspaceRelayGate, } from "../services/community-mode-contract";
...
import { mergeCoordinationTerminalMemberPubkeys } from "../services/community-participant-display-read-model";
import { useGroupHomeParticipantPubkeys } from "../hooks/use-group-home-participant-pubkeys";
...
import { loadCommunityProvisionalMemberPubkeys, stripProvisionalCommunityMembersConfirmedOnRelay, } from "../services/community-provisional-membership-cache";
import { clearCommunityTerminalMembershipEvidence, } from "../services/community-membership-evidence-actions";
...
import { loadCommunityTerminalMembershipCache, mergeTerminalMemberPubkeys, stripTerminalCommunityMembersWithActiveEvidence, } from "../services/community-terminal-membership-cache";
...
import { normalizeBotTriggersForDescriptor, readBotTriggersFromMetadataField, } from "../services/community-bot-triggers-policy";
...
interface GroupManagementDialogProps {
    ...
    myPrivateKeyHex: any;
    /** When provided, reuses the parent subscription instead of opening a second `useLegacySealedCommunity`. */
...
export function GroupManagementDialog({ isOpen, onClose, group, pool, myPublicKeyHex, myPrivateKeyHex, communityController, initialTab, }: GroupManagementDialogProps) {
    ...
    const initialMemberSeed = React.useMemo<ReadonlyArray<PublicKeyHex>>(() => getResolvedClientGateway().communityRoster.resolveSeedMemberPubkeysFromDirectory({
    ...
    const internalCommunity = useLegacySealedCommunity({
    ...
    const { state, updateMetadata, proposeDescriptorUpdate, proposeExpelMember, expelMemberDirect, sendVoteKick, castGovernanceVote, rotateRoomKey, refresh: refreshCommunityMembership, clearLocalTerminalMembershipEvidence, applyCoordinationSemanticMemberEvent, } = communityController ?? internalCommunity;
    const communityRelayTransportReady = React.useMemo(() => hasWritableCommunityRelayTransport(group.relayUrl), [group.relayUrl]);
    ...
    const getScopedMutedMembersKey = (groupId: string): string => (getScopedStorageKey(`obscur_group_muted_members_${groupId}`, getResolvedProfileId()));
    ...
    const managedWorkspaceRelayGate = React.useMemo(() => resolveManagedWorkspaceRelayGate({
        communityMode: group.communityMode,
    ...
    const requireManagedWorkspaceRelayGate = React.useCallback((): boolean => {
        if (!managedWorkspaceRelayGate.allowed) {
            toast.error(managedWorkspaceRelayGate.userMessage
    ...
    useEffect(() => {
        const saved = localStorage.getItem(getScopedMutedMembersKey(group.groupId))
    ...
    useEffect(() => {
        if (state.metadata) {
            setEditName(resolveCommunityDisplayName({
    ...
    const projectionMemberPubkeys = communityRosterByConversationId[group.id]?.activeMemberPubkeys as ReadonlyArray<PublicKeyHex> | undefined;
    const { activeMemberPubkeys: activeMembers, authorEvidencePubkeys } = React.useMemo(() => getResolvedClientGateway().communityRoster.resolveActiveMemberPubkeysFromConversation({
    ...
    const provisionalMemberPubkeys = React.useMemo(() => loadCommunityProvisionalMemberPubkeys({
        groupId: group.groupId,
        relayUrl: group.relayUrl,
        profileId: getResolvedProfileId(),
    }), [group.groupId, group.relayUrl, provisionalOverlayEpoch]);
    const mergedManagementMemberPubkeys = React.useMemo(() => Array.from(new Set([...activeMembers, ...provisionalMemberPubkeys])) as ReadonlyArray<PublicKeyHex>, [activeMembers, provisionalMemberPubkeys]);
    const communityModeForR1 = (state.metadata?.communityMode === "managed_workspace" || state.metadata?.communityMode === "sovereign_room"
        ? state.metadata.communityMode
        : group.communityMode);
    const directoryParticipantPubkeys = React.useMemo(() => (communityKnownParticipantDirectoryByConversationId[group.id]?.participantPubkeys ?? []) as ReadonlyArray<PublicKeyHex>, [communityKnownParticipantDirectoryByConversationId, group.id]);
    const relayEvidenceConfidence = (state as {
        relayEvidenceRef?: {
            confidenceLevel: "seed_only" | "warming_up" | "partial_eose" | "steady_state";
        };
    }).relayEvidenceRef?.confidenceLevel ?? "seed_only";
    const terminalMembershipCache = React.useMemo(() => loadCommunityTerminalMembershipCache({
        groupId: group.groupId,
        relayUrl: group.relayUrl,
        profileId: getResolvedProfileId(),
    }), [group.groupId, group.relayUrl]);
    const effectiveLeftMemberPubkeys = React.useMemo(() => mergeTerminalMemberPubkeys(terminalMembershipCache?.leftMemberPubkeys ?? [], state.leftMembers), [state.leftMembers, terminalMembershipCache]);
    const effectiveExpelledMemberPubkeys = React.useMemo(() => mergeTerminalMemberPubkeys(terminalMembershipCache?.expelledMemberPubkeys ?? [], state.expelledMembers), [state.expelledMembers, terminalMembershipCache]);
    ...
    const effectiveCommunityMode = React.useMemo(() => resolveEffectiveCommunityMode(communityModeForR1, group.relayUrl), [communityModeForR1, group.relayUrl]);
    const rosterLeftMemberPubkeys = React.useMemo(() => mergeCoordinationTerminalMemberPubkeys(effectiveLeftMemberPubkeys, coordinationMembershipDirectory, "left"), [coordinationMembershipDirectory, effectiveLeftMemberPubkeys]);
    const rosterExpelledMemberPubkeys = React.useMemo(() => mergeCoordinationTerminalMemberPubkeys(effectiveExpelledMemberPubkeys, coordinationMembershipDirectory, "expelled"), [coordinationMembershipDirectory, effectiveExpelledMemberPubkeys]);
    const {
        participantDisplayPubkeys,
        authorEvidencePubkeys: rosterAuthorEvidencePubkeys,
    ...
    const inviteEligibleMemberPubkeys = React.useMemo(() => (
        usesKernelRoster
            ...
            : resolveCommunityInviteMemberBlocklist({
                ...
                participationAuthorPubkeys: rosterAuthorEvidencePubkeys,
    ...
    const visibleMemberRegistry = React.useMemo(() => filterVisibleGroupMembers(participantDisplayPubkeys, (pubkey) => discoveryCache.getProfile(pubkey)), [participantDisplayPubkeys]);
    const onlineMemberCount = React.useMemo(() => visibleMemberRegistry.filter((pubkey) => presence.isPeerOnline(pubkey)).length, [visibleMemberRegistry, presence]);
    const operatorHealth = React.useMemo(() => summarizeCommunityOperatorHealth({
    ...
    const stewardPolicy = React.useMemo(() => resolveCommunityStewardPolicy({
        communityMode: state.metadata?.communityMode ?? group.communityMode,
    ...
    const handleReconcileMembership = React.useCallback(async () => {
        ...
        const outcome = await reconcileWorkspaceMembershipEvidence({
            ...
            communityMode,
            localMemberPubkey: myPublicKeyHex ?? undefined,
            actorPrivateKeyHex: myPrivateKeyHex ?? undefined,
    ...
    const terminalRecordCount = React.useMemo(() => new Set([...effectiveLeftMemberPubkeys, ...effectiveExpelledMemberPubkeys]).size, [effectiveExpelledMemberPubkeys, effectiveLeftMemberPubkeys]);
    ...
    const communityModeLabel = effectiveCommunityMode === "managed_workspace"
        ? t("groups.mode.managedWorkspace.label")
        : t("groups.mode.sovereignRoom.label");
    const syncConfidenceLevel = (state as {
        relayEvidenceRef?: {
            confidenceLevel: "seed_only" | "warming_up" | "partial_eose" | "steady_state";
        };
    })
    ...
    return (<>
            <GroupManagementShell isOpen={isOpen} onClose={onClose} relayGateNotice={<ManagedWorkspaceRelayGateBanner gate={managedWorkspaceRelayGate}/>} communityTitle={communityTitle} communityInitial={communityInitial} avatarUrl={state.metadata?.picture || editPicture || undefined} relayHost={relayHost} communityModeLabel={communityModeLabel} activeTab={activeTab} onTabChange={setActiveTab} governanceBadgeCount={activeProposalCount} headerAction={showShareInHeader ? (<Button type="button" variant="secondary" size="sm" onClick={() => {
                if (!requireManagedWorkspaceRelayGate()) {
                    return;
                }
                setIsQrModalOpen(true);
            }} disabled={managedWorkspaceActionsBlocked} className="rounded-lg border border-zinc-300 bg-white text-zinc-900 hover:bg-zinc-50 disabled:opacity-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100">
                            <QrCode className="mr-2 h-4 w-4"/>
                            {t("groups.management.shareInvite")}
                                        </Button>) : undefined} footer={activeTab === "general" && isAdmin ? (<div className="flex justify-end gap-3">
                            <Button type="button" variant="ghost" onClick={onClose} className="rounded-lg text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white">
                                {t("common.cancel")}
                                        ...
                                        <Button type="button" onClick={() => void handleSaveGeneral()} disabled={isSaving || managedWorkspaceActionsBlocked} className="rounded-lg bg-violet-600 px-6 hover:bg-violet-500">
                                {isSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : null}
                                {requiresMemberVote ? t("groups.management.proposeChanges") : t("groups.management.saveChanges")}
                                        </Button>
                                        </div>) : undefined}>
                {activeTab === "general" ? (<GroupManagementGeneralPanel editName={editName} setEditName={setEditName} editAbout={editAbout} setEditAbout={setEditAbout} editPicture={editPicture} editAccess={editAccess} setEditAccess={setEditAccess} isAdmin={isAdmin} isUploading={isUploading} onPickAvatar={() => void handlePickAvatar()} requiresMemberVote={requiresMemberVote} stewardPolicy={stewardPolicy} communityMode={group.communityMode} relayUrl={group.relayUrl} relayCapabilities={relayCapabilities} isRelayCapabilitiesLoading={isRelayCapabilitiesLoading} managedWorkspaceRelayGate={managedWorkspaceRelayGate} editBotPubkeys={editBotPubkeys} onEditBotPubkeysChange={setEditBotPubkeys} editBotTriggers={editBotTriggers} onEditBotTriggersChange={setEditBotTriggers} requiresGovernanceProposal={requiresMemberVote}/>) : null}

                {activeTab === "members" ? (<GroupManagementMembersPanel visibleMemberPubkeys={visibleMemberRegistry} relayBackedMemberPubkeys={activeMembers} provisionalMemberPubkeys={provisionalMemberPubkeys} memberSearchQuery={memberSearchQuery} setMemberSearchQuery={setMemberSearchQuery} resolvedNames={resolvedNames} onlineMemberCount={onlineMemberCount} operatorHealth={operatorHealth} myPublicKeyHex={myPublicKeyHex} isAdmin={isAdmin} mutedMembers={mutedMembers} kickingMemberPubkey={kickingMemberPubkey} currentTime={currentTime} onInvite={() => {
                if (!requireManagedWorkspaceRelayGate()) {
                    return;
                }
                setIsInviteModalOpen(true);
            }} onToggleMute={toggleMute} onVoteKick={(pubkey) => void handleVoteKick(pubkey)} syncConfidenceLevel={syncConfidenceLevel} isPoolConnected={pool !== null} terminalRecordCount={terminalRecordCount} onReconcileMembership={handleReconcileMembership} onClearTerminalMembership={handleClearTerminalMembership} managedWorkspaceActionsBlocked={managedWorkspaceActionsBlocked} directoryHonesty={directoryHonesty} membershipEvidenceUiContext={membershipEvidenceUiContext}/>) : null}

                {activeTab === "governance" ? (<GroupManagementGovernancePanel proposals={activeGovernanceProposals} myPublicKeyHex={myPublicKeyHex} describeProposal={describeGovernanceProposal} onVote={(params) => void castGovernanceVote(params)} managedWorkspaceActionsBlocked={managedWorkspaceActionsBlocked}/>) : null}

                {activeTab === "settings" ? (<GroupManagementSettingsPanel notificationsEnabled={notificationsEnabled} onToggleNotifications={toggleNotifications} onShareInvite={() => {
                if (!requireManagedWorkspaceRelayGate()) {
                    return;
                }
                setIsQrModalOpen(true);
            }} isRotatingKey={isRotatingKey} onRotateKey={() => void handleRotateKey()} onExport={() => void exportCommunity()} onLeave={openLeaveConfirmation} onPurge={openPurgeConfirmation} showPurge={isLocalAdmin || !communityRelayTransportReady} managedWorkspaceActionsBlocked={managedWorkspaceActionsBlocked}/>) : null}
                ...
                <DialogContent className="border-zinc-800 bg-zinc-950 sm:max-w-md">
                    ...
                    <div className="space-y-4 p-2">
                        <div className="overflow-hidden rounded-xl bg-white p-4">
                            <GroupQRCode groupId={group.groupId} relayUrl={group.relayUrl} groupName={state.metadata?.name || group.displayName} roomKeyHex={roomKeyHex}/>
                        </div>
                        <Button type="button" className="w-full rounded-lg bg-violet-600 hover:bg-violet-500" onClick={() => {
            ...
            <InviteMemberDialog isOpen={isInviteModalOpen} onClose={() => setIsInviteModalOpen(false)} groupId={group.groupId} relayUrl={group.relayUrl} roomKeyHex={roomKeyHex || ""} communityId={group.communityId} genesisEventId={group.genesisEventId} creatorPubkey={group.creatorPubkey} currentMemberPubkeys={inviteEligibleMemberPubkeys} metadata={{
            id: group.groupId,
