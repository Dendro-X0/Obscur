...
export function InvitationComposerDialog({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl overflow-hidden rounded-[32px] border border-white/10 bg-[#040816] text-white shadow-[0_30px_80px_rgba(0,0,0,0.45)]"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="border-b border-white/10 bg-[radial-gradient(circle_at_top,_rgba(99,102,241,0.28),_transparent_56%),linear-gradient(180deg,rgba(10,18,46,0.95),rgba(4,8,22,0.98))] px-6 py-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="mb-3 inline-flex w-fit items-center rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-slate-300">
                Invitation Composer
              </div>
          ...
          <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
            ...
            <p className="mt-1 text-sm font-semibold text-white">{recipientName}</p>
            <p className="mt-1 break-all font-mono text-[11px] text-slate-400">{recipientPubkey}</p>
          </div>
        </div>

        <div className="space-y-5 px-6 py-6">
          <div className="space-y-2">
            <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-300">Why you are reaching out</p>
            <Textarea
              value={intro}
              ...
              className="resize-none rounded-2xl border-white/10 bg-white/[0.04] text-white placeholder:text-slate-500"
            />
            <div className="flex justify-end text-[11px] text-slate-400">{intro.length}/280</div>
          </div>

          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-300">How they know you</p>
              <Input
                value={note}
            ...
            <div className="space-y-2">
              <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-300">Shared phrase or code</p>
              <Input
                value={secretCode}
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <Button
              variant="outline"
              className="rounded-2xl border-white/10 bg-transparent text-slate-200 hover:bg-white/5"
              onClick={onClose}
              disabled={isSubmitting}
            >
              Cancel
