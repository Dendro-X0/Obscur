...
export const SidebarUserSearch = ({ query, onQueryChange, inputRef, onUserSelect, dismissSignal }: SidebarUserSearchProps) => {
    ...
    return (<div ref={containerRef} className="relative w-full">
        ...
        }} placeholder={t("messaging.searchUnified")} className="w-full h-11 pl-9 pr-10 text-[11px] bg-black/[0.02] dark:bg-white/[0.02] border border-transparent focus:bg-white dark:focus:bg-zinc-900 focus:border-purple-500/20 rounded-2xl transition-all outline-none" onFocus={() => shouldSearchGlobalUsers && setShowResults(true)} suppressHydrationWarning data-testid="sidebar-unified-search-input"/>
                {isSearching ? (<Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 animate-spin text-purple-500/50"/>) : null}
            </div>

            {showResults && shouldSearchGlobalUsers ? (<div className="absolute top-full left-0 w-[280px] mt-2 bg-white dark:bg-zinc-900 border border-black/5 dark:border-white/5 rounded-[24px] shadow-2xl z-50 overflow-hidden max-h-80 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    {results.length === 0 && !isSearching ? (<div className="p-6 text-center text-xs text-zinc-500 italic">
                            {t("common.noUsersFound")}
                        </div>) : (<div className="p-1">
                            {results.map((user) => (<div role="button" tabIndex={0} key={user.pubkey} onClick={() => {
                        onUserSelect(user);
                        setShowResults(false);
                        onQueryChange("");
                    }} onKeyDown={(e) => {
                            ...
                            onQueryChange("");
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-1.5 overflow-hidden">
                                            ...
                                            {user.nip05 ? (<div className="flex h-3 w-3 items-center justify-center rounded-full bg-blue-500 text-[6px] text-white" title={user.nip05}>
                                                    V
                                                </div>) : null}
                                            {user.trustScore && user.trustScore > 0 ? (<div className="flex h-3 w-3 items-center justify-center rounded-full bg-amber-500 text-[6px] text-white">
                                                    *
                                                </div>) : null}
                                        </div>
                                        <div className="flex items-center gap-2 text-[10px] text-zinc-500 truncate">
                                            <span>{t("messaging.identityHidden")}</span>
                                            {user.mutuals && user.mutuals.length > 0 ? (<span className="flex items-center gap-0.5 text-purple-500 font-bold">
                                                    + {user.mutuals.length} {t("common.mutuals")}
                                                </span>) : null}
                                        </div>
                                    </div>
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity mr-1">
                                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-purple-100 dark:hover:bg-purple-900/30 text-purple-600" onClick={(e: React.MouseEvent) => {
                        e.stopPropagation();
                        onUserSelect(user);
                        setShowResults(false);
                        onQueryChange("");
                                        ...
                                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-purple-100 dark:hover:bg-purple-900/30 text-purple-600">
