...
export function VoiceCallInviteCard({ invite, isOutgoing, isJoining = false, onJoinCall, onRequestCallback, callbackConsumed = false, callSummary = null, nowUnixMs = null, liveStatusPhase = null, liveReasonCode = null, compact = false, }: VoiceCallInviteCardProps): React.JSX.Element {
    ...
    const missedWithoutFailure = !hasConnected && (liveReasonCode === "left_by_user"
        || liveReasonCode === "remote_left"
        || liveStatusPhase === "interrupted");
    const unconnectedTimeout = !callSummary?.endedNormally && !wasInterruptedByFailure && !missedWithoutFailure && !hasConnected && (inviteExpiredByNow || endedWithoutConnection || liveStatusPhase === "ended");
    ...
    const liveStatusLabel = (() => {
        switch (liveStatusPhase) {
            case "ringing_outgoing":
                return t("messaging.voiceCallRingingOutgoing");
            case "ringing_incoming":
                return t("messaging.voiceCallRingingIncoming");
            case "connecting":
                return t("messaging.voiceCallConnecting");
            case "connected":
                return t("messaging.voiceCallConnected");
            case "interrupted":
                return t("messaging.voiceCallInterrupted");
            case "ended":
                return t("messaging.voiceCallEnded");
            default:
                return null;
        }
    })();
    const stateSubtitle = (() => {
        switch (cardState) {
            case "completed":
                return t("messaging.voiceCallEndedNormally");
            case "missed":
                return isOutgoing
                    ? t("messaging.voiceCallNotAnswered")
                    : t("messaging.voiceCallMissedReturnCall");
            case "timed_out":
                return t("messaging.voiceCallTimedOutBeforeConnect");
            case "failed":
                return t("messaging.voiceCallFailedBeforeCompletion");
            default:
                return isOutgoing
                    ? t("messaging.voiceCallInviteSent")
                    : t("messaging.voiceCallInviteReceived");
        }
    })();
    const statusBadgeLabel = (() => {
        switch (cardState) {
            case "completed":
                return t("messaging.voiceCallCompleted");
            case "missed":
                return isOutgoing
                    ? t("messaging.voiceCallNotConnected")
                    : t("messaging.voiceCallMissed");
            case "timed_out":
                return t("messaging.voiceCallNoAnswer");
            case "failed":
                return t("messaging.voiceCallFailed");
            default:
        ...
        switch (cardState) {
            ...
            case "missed":
                return isOutgoing
                    ...
                    : t("messaging.voiceCallMissed");
            case "timed_out":
                return t("messaging.voiceCallTimedOut");
            case "failed":
                return t("messaging.voiceCallFailed");
            default:
                return t("messaging.voiceCallInvite");
        }
    })();
    ...
    const primaryActionLabel = cardState === "missed" || cardState === "timed_out"
        ? t("messaging.voiceCallCallback")
        : isJoining
            ? t("messaging.voiceCallJoining")
            : t("messaging.voiceCallJoin");
    return (<div data-testid="voice-call-invite-card" className={cn("relative overflow-hidden rounded-2xl border border-surface-contrast px-3.5 py-2.5 text-surface-contrast-primary w-full", compact ? "max-w-full" : "max-w-[420px]", "bg-gradient-surface-contrast shadow-[0_12px_30px_rgba(15,23,42,0.14)] dark:shadow-[0_14px_36px_rgba(0,0,0,0.46)]", containerToneClass)}>
      <div aria-hidden="true" className={cn("pointer-events-none absolute inset-x-0 top-0 h-14 bg-gradient-to-b", accentScrimClass)}/>
      <div className="relative flex items-center gap-2.5">
        <div className={cn("inline-flex h-6 w-6 items-center justify-center rounded-full", iconToneClass)}>
      ...
      {!compact || cardState === "completed" || cardState === "failed" ? (<div className="relative mt-2.5 grid grid-cols-[62px,1fr] items-center gap-x-2 gap-y-1 text-[12px] leading-4.5">
        {invitedAtLabel ? (<>
            <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-[0.12em] text-surface-contrast-secondary">
              ...
              {t("messaging.voiceCallInvitedAt")}
            </span>
        ...
        {expiresAtLabel ? (<>
            <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-surface-contrast-secondary">
              {t("messaging.voiceCallExpiresAt")}
            </span>
            <span className="truncate tabular-nums text-[12px] text-surface-contrast-primary">
              {expiresAtLabel}
            </span>
      ...
      <div className="relative mt-2.5 flex justify-end">
        {statusBadgeLabel ? (<span data-testid="voice-call-status-badge" className={cn("mr-auto inline-flex items-center rounded-full border px-2 py-1 text-[10px] font-black uppercase tracking-widest", badgeToneClass)}>
            {statusBadgeLabel}
          </span>) : null}
        {showJoinAction ? (<Button type="button" size="sm" className="h-8 rounded-lg px-3 text-[11px] font-bold" onClick={() => onJoinCall?.(invite)} disabled={isJoining}>
            {primaryActionLabel}
          </Button>) : null}
        {showCallbackAction ? (<Button type="button" size="sm" className="h-8 rounded-lg px-3 text-[11px] font-bold" onClick={onRequestCallback}>
            {primaryActionLabel}
          </Button>) : null}
        {(cardState === "missed" || cardState === "timed_out") && callbackConsumed ? (<span className="inline-flex items-center rounded-full border border-surface-contrast bg-black/5 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-surface-contrast-secondary dark:bg-white/5">
            {t("messaging.voiceCallCallbackUsed")}
          </span>) : null}
      </div>
    </div>);
}
