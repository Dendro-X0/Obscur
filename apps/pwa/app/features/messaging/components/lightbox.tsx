...
import { isChatAttachmentSavedToLocalVault, saveChatAttachmentToLocalVault } from "@/app/features/vault/services/save-chat-attachment-to-vault";
...
function LegacyLightbox({ item, onClose }: LightboxProps) {
    ...
    React.useEffect(() => {
        ...
        let cancelled = false;
        void (async () => {
            const saved = await isChatAttachmentSavedToLocalVault(item.attachment.url);
            if (!cancelled) {
    ...
    const onSaveToVault = async (event: React.MouseEvent) => {
        event.stopPropagation();
        if (!hasNativeRuntime() || isSavingToVault || isSavedToVault) {
            return;
        }
    ...
    return (<div data-escape-layer="open" className="media-preview-backdrop fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-xl" onPointerDown={onClose}>
      ...
      <div className="relative w-full max-w-5xl" onPointerDown={(event) => event.stopPropagation()}>
        <div className="absolute right-2 top-2 z-10 flex items-center gap-2">
          {hasNativeRuntime() ? (<button type="button" className="media-viewer-control" onClick={(event) => void onSaveToVault(event)} disabled={isSavingToVault || isSavedToVault} aria-label={t("vault.saveFromChat")} title={isSavedToVault ? t("vault.alreadyInVault") : t("vault.saveFromChat")}>
            <HardDrive className="h-4 w-4"/>
          </button>) : null}
          <button type="button" className="media-viewer-control" onClick={onDownload} aria-label={t("common.download")}>
        ...
        <div className="overflow-hidden rounded-2xl border border-zinc-300/65 bg-white/75 shadow-[0_24px_80px_rgba(15,23,42,0.24)] dark:border-white/10 dark:bg-black/75 dark:shadow-[0_28px_90px_rgba(0,0,0,0.6)]">
          {kind === "image" ? (<Image src={item.attachment.url} alt={item.attachment.fileName} width={1280} height={720} unoptimized className="h-auto w-full max-h-[90vh] object-contain"/>) : (kind === "audio" || kind === "voice_note") ? (<div className="p-6">
...
function V083Lightbox({ item, onClose, onPrev, onNext, hasPrev, hasNext, activeIndex, totalItems }: LightboxProps) {
    ...
    React.useEffect(() => {
        ...
        setIsSavedToVault(false);
        void (async () => {
            if (!hasNativeRuntime() || !item?.attachment.url) {
                return;
            }
            const saved = await isChatAttachmentSavedToLocalVault(item.attachment.url);
            setIsSavedToVault(Boolean(saved));
        })();
    ...
    return (<AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} data-escape-layer="open" className="media-preview-backdrop fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-xl md:p-8" onPointerDown={onClose}>
        <div aria-hidden className="media-preview-depth-layer absolute inset-0"/>
        <div className="absolute left-5 top-5 z-[120] max-w-[min(70vw,24rem)] rounded-[28px] border border-zinc-300/70 bg-white/88 px-4 py-3 shadow-xl backdrop-blur-xl dark:border-white/15 dark:bg-black/45" onPointerDown={(event) => event.stopPropagation()}>
        ...
        <div className="absolute top-5 right-5 z-[120] flex items-center gap-2 rounded-2xl border border-zinc-300/70 bg-white/85 p-2 shadow-xl backdrop-blur-xl dark:border-white/15 dark:bg-black/45" onPointerDown={(e) => e.stopPropagation()}>
          {kind === "image" ? (<>
        ...
        {kind === "image" ? (<div className="relative flex h-full w-full items-center justify-center overflow-hidden rounded-3xl border border-zinc-300/60 bg-white/55 shadow-[0_30px_100px_rgba(15,23,42,0.2)] dark:border-white/10 dark:bg-black/60 dark:shadow-[0_36px_110px_rgba(0,0,0,0.62)]" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => {
            ...
            <motion.div drag={zoom > 1} dragElastic={0.08} dragMomentum style={{ x, y, scale: zoom }} className={cn("flex items-center justify-center", zoom > 1 ? "cursor-grab active:cursor-grabbing" : "")}>
          ...
          </div>) : (kind === "audio" || kind === "voice_note") ? (<div className="w-full max-w-2xl rounded-3xl border border-zinc-300/60 bg-white/70 p-8 shadow-[0_28px_90px_rgba(15,23,42,0.22)] dark:border-white/10 dark:bg-black/70 dark:shadow-[0_30px_95px_rgba(0,0,0,0.58)]" onPointerDown={(event) => event.stopPropagation()}>
            <AudioPlayer src={item!.attachment.url} isOutgoing={false} voiceNoteMetadata={getVoiceNoteAttachmentMetadata(item!.attachment)}/>
          </div>) : (kind === "file" && isPdfAttachment(item!.attachment)) ? (<div className="w-full max-w-6xl overflow-hidden rounded-3xl border border-zinc-300/60 bg-white/70 p-4 shadow-[0_28px_90px_rgba(15,23,42,0.22)] dark:border-white/10 dark:bg-black/70 dark:shadow-[0_30px_95px_rgba(0,0,0,0.58)]" onPointerDown={(event) => event.stopPropagation()}>
          ...
          </div>) : (<div className="w-full max-w-6xl overflow-hidden rounded-3xl border border-zinc-300/60 bg-white/70 shadow-[0_28px_90px_rgba(15,23,42,0.22)] dark:border-white/10 dark:bg-black/70 dark:shadow-[0_30px_95px_rgba(0,0,0,0.58)]" onPointerDown={(event) => event.stopPropagation()}>
        ...
        {hasSequence ? (<div className="absolute bottom-5 left-1/2 z-[120] flex w-[min(calc(100vw-2rem),28rem)] -translate-x-1/2 items-center gap-2 rounded-[28px] border border-zinc-300/70 bg-white/90 p-2 shadow-2xl backdrop-blur-2xl dark:border-white/15 dark:bg-black/50" onPointerDown={(event) => event.stopPropagation()}>
