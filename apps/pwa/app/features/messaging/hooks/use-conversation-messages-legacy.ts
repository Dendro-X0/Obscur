...
/**
 * Legacy DM thread messages hook — routed via `use-thread-messages` when legacy opt-in.
 */
...
import { messageBus, type MessageBusEvent } from "@/app/features/messaging/services/message-bus";
import { PrivacySettingsService } from "@/app/features/settings/services/privacy-settings-service";
import { performanceMonitor } from "@/app/features/messaging/lib/performance-monitor";
...
import { isCommunityInviteThreadPayloadContent } from "@/app/features/messaging/services/dm-community-invite-thread-payload";
import {
...
import { expandDmDeleteIdsForThread } from "@/app/features/messaging/services/expand-dm-delete-ids-for-thread";
...
import { areMessageListsEquivalentById } from "@/app/features/messaging/services/dm-conversation-message-list-equiv";
import {
...
import { inferPeerFromConversationId, buildDmSiblingConversationIds } from "@/app/features/messaging/utils/dm-conversation-sibling-ids";
...
} from "@/app/features/messaging/services/dm-thread-display-cache";
import { loadDmThreadSyncSeedCache } from "@/app/features/messaging/services/dm-thread-sync-seed-loader";
import {
    cancelCoalescedConversationHydrate,
    scheduleCoalescedConversationHydrate,
} from "@/app/features/messaging/services/conversation-hydrate-coordinator";
import {
    applyDmRedactionDisplayGateAsync,
    filterMessagesThroughDmRedactionDisplayGate,
    subscribeDmRedactionDisplayGateChanged,
} from "@/app/features/messaging/services/dm-redaction-display-gate";
 ...
 */
export function useLegacyConversationMessages(
    ...
    const legacyChatStateHasRicherDmContent = useMemo(() => {
        ...
        return aliasIds.some((aliasId) => (
            profileIds.some((profileId) => {
                const persistedState = messagingChatStateReadPort.load(publicKeyHex as PublicKeyHex, { profileId });
    ...
    const hydrateHistory = useCallback(async (
        ...
        try {
            ...
            if (isNativeDmSqliteReadOwner()) {
                const nativeResult = await runLegacyNativeDmConversationHistoryHydrate({
                    conversationId: cid,
                    conversationIds,
                    profileId: activeProfileId?.trim() || getResolvedProfileId() || undefined,
                    messageDeleteTombstones,
    ...
    useEffect(() => {
        ...
        const nativeStaleEmpty = isNativeDmSqliteReadOwner();
        const retryPolicy = evaluateStaleEmptyHydrateRetryPolicy({
            ...
            maxAttempts: nativeStaleEmpty
                ? NATIVE_DM_THREAD_STALE_EMPTY_HYDRATE_MAX_ATTEMPTS
                : DM_THREAD_STALE_EMPTY_HYDRATE_MAX_ATTEMPTS,
            baseDelayMs: nativeStaleEmpty
                ? NATIVE_DM_THREAD_STALE_EMPTY_HYDRATE_BASE_DELAY_MS
                : DM_THREAD_STALE_EMPTY_HYDRATE_BASE_DELAY_MS,
        });
    ...
    useEffect(() => {
        if (!isNativeDmSqliteReadOwner() || !conversationId || !isDmThread) {
            return;
        }
        const onProfileBootReconciled = (): void => {
            staleEmptyHydrateAttemptRef.current = 0;
            requestConversationHydrate("profile_boot_reconciled", {
                onlyIfEmpty: true,
    ...
    }, [conversationId, isDmThread, requestConversationHydrate]);
