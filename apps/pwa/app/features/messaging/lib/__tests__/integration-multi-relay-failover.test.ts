...
vi.mock('@/app/features/crypto/crypto-service', () => ({
  cryptoService: {
    ...
    encryptGiftWrap: vi.fn(async (_recipient, event) => ({
      content: JSON.stringify(event),
      kind: 1059,
      tags: [['p', _recipient]],
      created_at: Math.floor(Date.now() / 1000),
      pubkey: 'random_seal_key'
    })),
    decryptGiftWrap: vi.fn(async (event) => {
      // Mock unwrapping: just return the inner content
...
describe('Integration: Multi-Relay Failover and Recovery', () => {
  ...
  describe('Multi-Relay Publishing', () => {
    it('should publish to all connected relays', async () => {
      ...
      expect(sendResult.relayResults.every((r: { success: boolean }) => r.success)).toBe(true);
    });
    ...
    it('should succeed if at least one relay accepts', async () => {
      ...
      expect(sendResult.success).toBe(true);
      expect(sendResult.relayResults.some((r: { success: boolean }) => r.success)).toBe(true);

      // Verify message status is 'accepted'
      await waitFor(() => {
        const messages = result.current.state.messages;
        const message = messages.find((m: { id: string }) => m.id === sendResult.messageId);
  ...
  describe('Individual Relay Failures', () => {
    it('should continue with other relays when one fails', async () => {
      ...
      expect(sendResult.success).toBe(true);
      expect(sendResult.relayResults.filter((r: { success: boolean }) => r.success).length).toBe(2);
    });
    ...
    it('should track relay-specific failures', async () => {
      ...
      const relay1Result = sendResult.relayResults.find((r: { relayUrl: string }) => r.relayUrl === 'wss://relay1.example.com');
      const relay2Result = sendResult.relayResults.find((r: { relayUrl: string }) => r.relayUrl === 'wss://relay2.example.com');
  ...
  describe('All Relays Fail', () => {
    it('should queue message when all relays fail', async () => {
      ...
      await waitFor(() => {
        const messages = result.current.state.messages;
        const message = messages.find((m: { content: string }) => m.content === 'Test message');
  ...
  describe('Relay Performance Tracking', () => {
    it('should track relay latency', async () => {
      ...
      const relay1 = sendResult.relayResults.find((r: { relayUrl: string; latency?: number }) => r.relayUrl === 'wss://relay1.example.com');
      const relay2 = sendResult.relayResults.find((r: { relayUrl: string; latency?: number }) => r.relayUrl === 'wss://relay2.example.com');
  ...
  describe('Subscription Management Across Relays', () => {
    it('should subscribe to all open relays', async () => {
      ...
      const reqCall = calls.find((call: any[]) => {
        ...
        try {
          if (!message.includes('REQ')) return false;
          const parsed = JSON.parse(message);
          const filter = parsed[2];
          return filter && filter.kinds && filter.kinds.includes(4);
        } catch (e) {
          return false;
        }
      });
    ...
    it('should receive messages from any connected relay', async () => {
      ...
      await waitFor(() => {
        const messages = result.current.state.messages;
        expect(messages.some((m: { content: string }) => m.content === 'From relay 1')).toBe(true);
        expect(messages.some((m: { content: string }) => m.content === 'From relay 2')).toBe(true);
      }, { timeout: 2000 });
    });
  });
});
