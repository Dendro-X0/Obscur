...
export function DeviceTrustControl({
  profileId,
  checked,
  onCheckedChange,
  variant = "compact",
  idPrefix = "device-trust",
  className,
}: DeviceTrustControlProps): React.JSX.Element {
  ...
  const restoreHint = snapshot.usesNativeSecureStore
    ? t(
      "auth.deviceTrust.subtitle.native",
      "Uses this device’s secure storage. Your password is not saved in the browser.",
    )
    : t(
      "auth.deviceTrust.subtitle.web",
      "Saves an unlock token in this browser only. Use a private window if this is a shared computer.",
    );
  ...
  if (variant === "card") {
    return (
      <div
        className={cn(
          "rounded-[28px] border border-emerald-500/20 bg-emerald-500/5 p-4 text-left",
          className,
        )}
      >
          ...
          <div className="min-w-0 flex-1 space-y-3">
            <div className="flex items-start gap-3">
              <Checkbox
                ...
                className="mt-1 h-5 w-5 rounded-lg border-zinc-300 dark:border-zinc-700 data-[state=checked]:bg-emerald-600"
              />
              <div className="space-y-1">
                <label htmlFor={inputId} className="cursor-pointer text-sm font-bold text-zinc-800 dark:text-zinc-100">
                  {t("auth.deviceTrust.title", "Trust this device")}
                </label>
            ...
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-400 dark:text-zinc-500">
              {checked
                ? t("auth.deviceTrust.status.enabled", "Auto-unlock enabled for this profile on this device")
                : t("auth.deviceTrust.status.disabled", "You will unlock manually each time you open Obscur")}
            </p>
          </div>
        </div>
      </div>
    ...
    <div className={cn("space-y-1 px-2", className)}>
      <div className="flex items-start gap-3">
        <Checkbox
          ...
          checked={checked}
          onCheckedChange={(value) => onCheckedChange(value === true)}
          className="mt-0.5 h-5 w-5 rounded-lg border-zinc-300 dark:border-zinc-700 data-[state=checked]:bg-purple-600"
        />
