...
import { ChevronDown, ChevronUp, Info, ShieldAlert, X } from "lucide-react";
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-semibold text-amber-900/80 hover:bg-amber-500/10 hover:text-amber-950 dark:text-amber-200/80 dark:hover:text-amber-50"
              aria-label={t("messaging.trust.dismiss")}
            >
              <span className="hidden sm:inline">{t("messaging.trust.dismiss")}</span>
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
...
export function DmKernelTrustInfoStrip(props: Readonly<{
  assessment: DmTrustAssessment;
  onDismiss: () => void;
}>): React.JSX.Element | null {
  const { t } = useTranslation();
  if (props.assessment.tier !== "info") {
    return null;
  }

  return (
    <div
      data-testid="dm-kernel-trust-info-strip"
      data-trust-tier="info"
      className="z-10 px-4 py-2 border-b border-sky-500/15 bg-sky-500/5 backdrop-blur-sm"
      role="status"
      aria-live="polite"
    >
      <div className="max-w-4xl mx-auto flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0 text-xs text-sky-900/85 dark:text-sky-100/85">
          <Info className="h-3.5 w-3.5 shrink-0" aria-hidden />
          <span className="font-medium truncate">{t(props.assessment.copyKey)}</span>
        </div>
        <button
          type="button"
          onClick={props.onDismiss}
          className="shrink-0 rounded-md px-2 py-1 text-[11px] font-semibold text-sky-800/70 hover:bg-sky-500/10 dark:text-sky-100/70"
          aria-label={t("messaging.trust.dismiss")}
        >
