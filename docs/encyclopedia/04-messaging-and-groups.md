...
- Durable governance state should come from signed community events reduced by `community-ledger-validator.ts`, not from local optimistic UI state.
...
4. `community-ledger-validator.ts` computes membership and governance truth from ingestable event history.
