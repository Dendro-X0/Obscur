...
: Native command registration and owners:
  - `apps/desktop/src-tauri/src/lib.rs` - Main entry point and plugin initialization
  - `apps/desktop/src-tauri/src/commands/` - Tauri command handlers (modularized):
    - `mod.rs` - Module declarations
    - `window.rs` - Window management commands
    - `notification.rs` - Notification commands
    - `system.rs` - System/storage commands
    - `session.rs` - Session management commands
    - `tor.rs` - Tor proxy commands
    - `profile.rs` - Profile management commands
    - `tray.rs` - System tray commands
  - `apps/desktop/src-tauri/src/services/` - Helper services:
    - `mod.rs` - Module declarations
    - `tray.rs` - Tray menu, badge rendering, call state helpers
  - `apps/desktop/src-tauri/src/models/` - Shared types:
    - `mod.rs` - Module declarations
    - `app.rs` - App-related types (ResetAppStorageReport)
    - `tray.rs` - Tray state types (TrayCallState, TrayBadgeState, constants)
    - `tor.rs` - Tor configuration types
    - `window.rs` - Window state types
  - `apps/desktop/src-tauri/src/net.rs` - Network runtime
  - `apps/desktop/src-tauri/src/relay.rs` - Relay connection management
  - `apps/desktop/src-tauri/src/session.rs` - Native session storage
  - `apps/desktop/src-tauri/src/protocol.rs` - Protocol handlers
  - `apps/desktop/src-tauri/src/profiles.rs` - Profile management
  - `apps/desktop/src-tauri/src/upload.rs` - File upload handling
