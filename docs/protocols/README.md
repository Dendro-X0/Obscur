# Protocols Shelf

_Last reviewed: 2026-04-22 (baseline commit a3f16b10)._

Use this shelf for technical protocol and projection contracts.

Recommended reading order:

1. [21 Relay Transport Fault-Tolerance Spec](./21-relay-transport-fault-tolerance-spec.md)
2. [22 Local-First Decentralized Protocol Architecture](./22-local-first-decentralized-protocol-architecture.md)
3. [23 Private Direct Envelope and Community Room-Key Contract](./23-private-direct-envelope-and-community-room-key-contract.md)
4. [24 DM History Sync Stabilization Plan](./24-dm-history-sync-stabilization-plan.md)
5. [25 Community Ledger and Projection Architecture Spec](./25-community-ledger-and-projection-architecture-spec.md)
6. [26 Community Projection Contract](./26-community-projection-contract.md)
7. [27 Community Control and Governance Event Family](./27-community-control-and-governance-event-family.md)

**Product / UI:** [Community system — implementation and UI plan](../program/community-system-implementation-and-ui-plan.md) · [Phased roadmap by version](../program/community-system-overhaul-phased-roadmap.md).
