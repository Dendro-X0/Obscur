...
export type M6VoiceCaptureBundle = Readonly<{
  ...
  voice: Readonly<{
    ...
    connectingWatchdogIncidentBundleEvents: ReadonlyArray<MinimalAppEvent>;
    connectingWatchdogIncidentGateEvents: ReadonlyArray<MinimalAppEvent>;
...
const readRecentConnectingWatchdogIncidentBundleEvents = (
  appEventsApi: MinimalAppEventsApi | undefined,
): ReadonlyArray<MinimalAppEvent> => {
  try {
    if (typeof appEventsApi?.findByName !== "function") {
    ...
    return appEventsApi.findByName("messaging.realtime_voice.connecting_watchdog_incident_bundle", EVENT_CAPTURE_LIMIT) ?? [];
  } catch {
    return [];
  }
};

const readRecentConnectingWatchdogIncidentGateEvents = (
  appEventsApi: MinimalAppEventsApi | undefined,
): ReadonlyArray<MinimalAppEvent> => {
  try {
    if (typeof appEventsApi?.findByName !== "function") {
    ...
    return appEventsApi.findByName("messaging.realtime_voice.connecting_watchdog_incident_gate", EVENT_CAPTURE_LIMIT) ?? [];
  } catch {
    return [];
  }
};

const readRecentConnectingWatchdogIncidentGateEvidenceEvents = (
  appEventsApi: MinimalAppEventsApi | undefined,
): ReadonlyArray<MinimalAppEvent> => {
  try {
    if (typeof appEventsApi?.findByName !== "function") {
    ...
    return appEventsApi.findByName("messaging.realtime_voice.connecting_watchdog_incident_gate_evidence", EVENT_CAPTURE_LIMIT) ?? [];
  } catch {
    return [];
  }
};

const readRecentConnectingWatchdogIncidentGateSelfTestEvents = (
  appEventsApi: MinimalAppEventsApi | undefined,
): ReadonlyArray<MinimalAppEvent> => {
  try {
    if (typeof appEventsApi?.findByName !== "function") {
    ...
    return appEventsApi.findByName("messaging.realtime_voice.connecting_watchdog_incident_gate_self_test", EVENT_CAPTURE_LIMIT) ?? [];
  } catch {
    return [];
  }
};

const readRecentConnectingWatchdogIncidentGateCloseoutEvents = (
  appEventsApi: MinimalAppEventsApi | undefined,
): ReadonlyArray<MinimalAppEvent> => {
  try {
    if (typeof appEventsApi?.findByName !== "function") {
    ...
    return appEventsApi.findByName("messaging.realtime_voice.connecting_watchdog_incident_gate_closeout", EVENT_CAPTURE_LIMIT) ?? [];
  } catch {
    return [];
  }
};
...
const createBundle = (
  ...
  return {
    ...
    voice: {
      ...
      connectingWatchdogIncidentBundleEvents: readRecentConnectingWatchdogIncidentBundleEvents(appEventsApi),
      connectingWatchdogIncidentGateEvents: readRecentConnectingWatchdogIncidentGateEvents(appEventsApi),
