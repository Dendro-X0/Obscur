...
export const RATE_LIMITS = {
  ...
  CONNECTION_REQUEST_SEND: {
    maxOperations: 50,
    windowMs: 60 * 60 * 1000, // 50 connection requests per hour
  },
  INVITE_PROCESSING: {
    maxOperations: 30,
...
/**
 * Check if connection request sending is allowed
 */
export function canSendConnectionRequest(userId: string): boolean {
  return rateLimiter.isAllowed(
    `connection-req-${userId}`,
    RATE_LIMITS.CONNECTION_REQUEST_SEND.maxOperations,
    RATE_LIMITS.CONNECTION_REQUEST_SEND.windowMs
  );
}
